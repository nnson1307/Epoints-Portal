var reportSale = {
    ...
    getTotal : function () {  
        $.ajax({
            url: laroute.route('report-sale.get-total'),
            method: "POST",
            data: {
                time: $('#time').val(),
                branch: $('#branch').val(),
    ...
    chartSaleAmount: function(global = true){
        $.ajax({
            url: laroute.route('report-sale.get-chart-total'),
            method: "POST",
            global: global,
            data: {
                time: $('#time-chart').val(),
                branch: $('#branch-chart').val(),
                type: $('#type-chart').val(),
            },
            dataType: "JSON",
            success: function (data) {
    ...
    chartSaleAmountByBranch: function(global = true){
        $.ajax({
            url: laroute.route('report-sale.get-chart-total-by-branch'),
            method: "POST",
            global: global,
            data: {
                time: $('#time-chart').val(),
    ...
    chartTotalOrder: function(global = true){
        $.ajax({
            url: laroute.route('report-sale.get-chart-total-order'),
            method: "POST",
            data: {
                time: $('#time-chart').val(),
                branch: $('#branch-chart').val(),
                    ...
                    $('#container-total-order').highcharts({
                        ...
                        tooltip: {
                            formatter: function() {
                                ...
                                formatStr += '<span style="color:' + this.color + '">●</span>' + reportSale.jsonLang['Chi nhánh'] + ' ' + this.series.name + ': <b>' + formatNumber(this.y) + '</b><span></span><br/>';
    ...
    ChartOrdersByBranch: function (global = true) {
        $.ajax({
            url: laroute.route('report-sale.get-chart-total-order-by-branch'),
            method: "POST",
            data: {
                time: $('#time-chart').val(),
                branch: $('#branch-chart').val(),
    ...
    showChartSaleAmountByBranch: function (dataseries, categories) {
        ...
        $('#container-total-amount-branch').highcharts({
            ...
            tooltip: {
                formatter: function() {
                    var formatStr = '<span style="color:' + this.color + '">●</span><b>' + reportSale.jsonLang['Chi nhánh'] + ' ' + this.x + '</b><br />';
        ...
        $('#container-total-order-branch').highcharts({
            ...
            plotOptions: {
                pie: {
                    ...
                    dataLabels: {
                        enabled: true,
                        format: '<span style="color:{point.color}">●</span>' + reportSale.jsonLang['Chi nhánh'] + ' {point.name}: {point.y:.0f}%'
                    },
                    showInLegend: true
                }
            },
            ...
            tooltip: {
                headerFormat: '',
                pointFormat: '<span style="color:{point.color}">' + reportSale.jsonLang['Chi nhánh'] + ' {point.name}</span>: <b>{point.y:.0f}%</b>'
            },
...
$(document).ready(function (){
    ...
    var arrRange = {};
        ...
        $('#branch-chart').select2().on('select2:select', function (event) {
            reportSale.chartSaleAmount();
            reportSale.chartTotalOrder();
            reportSale.chartSaleAmountByBranch();
            reportSale.ChartOrdersByBranch();
