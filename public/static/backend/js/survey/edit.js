var survey = {
    init: function () {
        $(document).ready(function () {
            ...
            // kiểm tra hiển thị hoặc ẩn thời gian thực hiện khảo sát //

            let checkTimeShowSuvey = $('.frequency:checked').val();
            if (checkTimeShowSuvey == 'daily') {
                $('.frequency_weekly').hide();
                $('.frequency_monthly').hide();
            } else if (checkTimeShowSuvey == 'weekly') {
                $('.frequency_monthly').hide();
            } else if (checkTimeShowSuvey == 'monthly') {
                $('.frequency_weekly').hide();
            }
    ...
    update: function (survey_id) {
        $.getJSON(laroute.route('survey.validation'), function (json) {
            ...
            $.ajax({
                url: laroute.route('survey.update'),
                method: 'POST',
                dataType: 'JSON',
                data: data,
                success: function (res) {
                    if (res.error == false) {
                        swal.fire(json.edit_success, "", "success").then(function () {
                            window.location.href = laroute.route('survey.show', { id: survey_id });
                        });
                    } else {
                        var mess_error = '';
    ...
    showModalDestroy: function () {
        $('#destroy_survey').modal('show');
    },
    showModalConfirm: function () {
        $('#confirm_survey').modal('show');
    },
    showModalRefuse: function () {
        $('#refuse-survey').modal('show');
    },
    showModalEnd: function () {
        $('#end_survey').modal('show');
    },
    showModalPause: function () {
        $('#pause_survey').modal('show');
    },

    showModalContinue: function () {
        $('#continue_survey').modal('show');
    },
    ...
    destroy: function (id) {
        $.ajax({
            ...
            data: { id: id },
            success: function () {
                $('#destroy_survey').modal('hide');
                swal.fire('Xoá thành công', "", "success").then(function () {
                    window.location.href = laroute.route('survey.index');
                });
            },
            error: function (res) {
    ...
    changeStatus: function (id, status) {
        ...
        $.getJSON(laroute.route('survey.validation'), function (json) {
            ...
            if (status === 'R') {
                ...
                title = jsonLang['Tạm dừng khảo sát thành công'];
            } else if (status === 'continue') {
                title = jsonLang['Xác nhận tiếp tục chương trình thành công'];
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.error == false) {
                        swal.fire(`${title}`, "", "success").then(function () {
                            location.reload();
                        });
