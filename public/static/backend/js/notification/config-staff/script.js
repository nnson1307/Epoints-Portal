var index = {
    ...
    showEdit: function (key) {
        $.ajax({
            url: laroute.route('config-staff.edit'),
            method: 'POST',
            dataType: 'JSON',
            data: {
                key: key
    ...
    save: function (key) {
        ...
        if (continute == true) {
            $.ajax({
                url: laroute.route('config-staff.update'),
                method: 'POST',
                dataType: 'JSON',
                data: {
                    key: key,
                    ...
                    if (res.error == false) {
                        swal(res.message, "", "success").then(function (result) {
                            if (result.dismiss == 'esc' || result.dismiss == 'backdrop') {
                                window.location.href = laroute.route('config-staff');
                            }
    ...
    changeStatus: function (key, obj) {
        ...
        $.ajax({
            url: laroute.route('config-staff.change-status'),
            method: "POST",
            dataType: "JSON",
            data: {
                key: key,
