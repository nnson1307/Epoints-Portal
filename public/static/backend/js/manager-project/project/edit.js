...
var ProjectEdit = {
    init: () => {
        ...
        if ($("#project_type_customer").val() != '') {
            ProjectEdit.getCustomerDynamic($("#project_type_customer"));
        }
            ...
            success: function (res) {
                if (res.error == false) {
                    $.each(res.data, function (key, value) {
                        option += `<option ${customerSelected == value.customer_id ? 'selected' : ''}  value="${value.customer_id}">${value.full_name}</option>`;
                    })
    ...
    save: () => {
        ...
        form.validate({
            ...
            messages: {
                project_name: {
                    ...
                    maxlength: jsonLang['Nhập tối đa 50 kí tự'],
                },
                project_manager: {
        ...
        // gửi data
        $.ajax({
            url: laroute.route('manager-project.project.update'),
            data: data,
            method: "POST",
            dataType: "JSON",
            success: function (res) {
                if (res.error == false) {
                    swal.fire(jsonLang['Cập nhật dự án thành công'], "", "success").then(function (result) {
                            window.location.href = laroute.route('manager-project.project.project-info-overview',{id : $("#manage_project_id").val()});
                    });
                } else {
                    var mess_error = '';
                    ...
                    swal.fire(jsonLang['Cập nhật dự án thất bại'], mess_error, "error");
                }
            },
            error: function (res) {
