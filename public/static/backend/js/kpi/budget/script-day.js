$('#autotable').PioTable({
    baseUrl: laroute.route('kpi.marketing.budget.day.list')
});
...
var BudgetMarketingByDay = {
    remove: function (obj, id) {
        // hightlight row
        $(obj).closest('tr').addClass('m-table__row--danger');
        swal({
            title: jsonLang['Thông báo'],
            text: jsonLang['Bạn xác nhận muốn xóa ngân sách ngày này?'],
            type: 'warning',
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.error == 0) {
                            ...
                            window.location = laroute.route('kpi.marketing.budget.day');
                        } else {
    ...
    $('#frm-edit-criteria').submit(function (e) {
        ...
        form.validate({
            ...
            messages: {
                ...
                budget: {
                    required: jsonLang['Ngân sách ngày là trường bắt buộc phải nhập'],
                    maxlength: jsonLang['Ngân sách ngày chỉ cho phép tối đa là 999,999,999 VND']
                },
                effect_time: {
                    required: jsonLang['Hãy chọn ngày áp dụng'],
                }
            }
        });
