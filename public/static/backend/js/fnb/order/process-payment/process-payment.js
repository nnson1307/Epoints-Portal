...
processPayment = {
    ...
    editPaymentAction: function (type = 'order') {
            ...
            $.each($('#table_add').find(".tr_table"), function () {
                ...
                var is_change_price = $(this).find('input[name="is_change_price"]').val();
                var is_check_promotion = $(this).find('input[name="is_check_promotion"]').val();
                var id_detail = $(this).find('input[name="id_detail"]').val();
                var key_string = $(this).find('input[name="key_string"]').val();
                if (amount < 0) {
                    $('.error-table').text( processPayment.jsonLang['Tổng tiền không hợp lệ']);
                    continute = false;
            ...
            if (type == 'order') {
                if (table_subbmit == '' && table_add == '') {
                    ...
                    $('#receipt-btn').click(function () {
                        ...
                        if (check == true) {
                            $.ajax({
                                url: laroute.route('fnb.orders.submit-receipt-after'),
                                dataType: 'JSON',
                                data: {
            ...
            if (type == 'order') {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.table_error == 1) {
                                $('.error-table').text(processPayment.jsonLang['Vui lòng chọn sản phẩm']);
                                res({
                                    'status': false
                                });
                            }
                ...
                if (table_subbmit == '') {
                    ...
                    $('#receipt-btn').click(function () {
                        ...
                        if (check == true) {
                            $.ajax({
                                url: laroute.route('fnb.orders.submitAddReceipt'),
                                data: {
                                    order_id: $('#order_id').val(),
                                    order_code: $('#order_code').val(),
                                    staff_id: $('#staff_id').val(),
                                    ...
                                    if (response.error == true) {
                                        ...
                                        if (response.print_card.length > 0) {
                                            ...
                                            if(typeof $('#view_mode') != 'undefined' && $('#view_mode').val() == 'chathub_popup'){
                                                ...
                                                window.location = laroute.route('fnb.orders');
                                            }
                ...
                const resultPromise = new Promise((res, rej) => {
                    ...
                    $.each($('#table_add').find(".tr_table"), function () {
                        ...
                        table_row["voucher_code"] = $(this).find($("input[name='voucher_code']")).val();
                        table_row["amount"] = $(this).find($("input[name='amount']")).val();
                        table_row["staff_id"] = $(this).find($("[name='staff_id']")).val();
