...
var userGroupDefine = {
    ...
    removeRowTr: function (t, id2, page) {
    ...
    import: function () {
        $.getJSON(laroute.route('translate'), function (json) {
            if ($('#show').val() == '') {
                $('.error-input-excel').text(json['Vui lòng chọn file']);
            } else {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.success == 10) {
                            swal.fire(json['Thêm thất bại'], '', "error");
                        } else {
                            var success = 0;
                            ...
                            let mess = '';
                            mess = mess.concat(json['Số lượng khách hàng thành công'] + (success) + '<br/>');
                            mess = mess.concat(json['Số lượng khách hàng thất bại'] + (res.total - success) + '<br/>');
                            swal.fire(json['Thêm thành công'], mess, "success");
                        }
                    }
                });
            }
    ...
    searchWhereInUser: function (arrayUser, phone, fullName, isActive) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('.check-box-choose-user1').each(function () {
                    var p = $(this).parents('label').find('.customer-id-1').val();
            ...
            if (name == '') {
                ...
                if (name.length > 255) {
                    ...
                    if (arrayCustomerId.length == 0) {
                        ...
                        $.ajax({
                            url: laroute.route('admin.customer-group-filter.submit-add-group-define'),
                            method: "POST",
                            data: {
                                name: name,
                                arrayAccount: arrayCustomerId
                                ...
                                if (res.error == false) {
                                    swal.fire(json['Thêm thành công!'], '', "success").then(function (result) {
                                        if (result.dismiss == 'esc' || result.dismiss == 'backdrop') {
                                            if (ROUTE != '' && ID_SURVEY != '') {
                                                window.location.href = laroute.route('survey.edit-branch', { id: ID_SURVEY, type: 'customer_define' });
                                            } else if (type == 0) {
