...
function chartGrowthServiceCardGroup(dataValue) {
    google.charts.load("current", {packages: ["corechart"]});
    google.charts.setOnLoadCallback(drawChart);
...
function index() {
    $.ajax({
        url: laroute.route('admin.report-growth.service-card.index'),
        method: "POST",
        data: {time: $('#time').val()},
        dataType: "JSON",
        success: function (data) {
            $.getJSON(laroute.route('translate'), function (json) {
            //Biểu đồ sử dụng thẻ dịch vụ
            chartGrowthServiceCard(data['dataQuantity']);

            //Biểu đồ khách hàng(vãng lai và thành viên)
            var dataCustomer = [
                ['Task', 'Hours per Day'],
                [json['Thành viên'], data['totalQuantity'] - data['totalQuantityOddCustomer']],
                [json['Khách hàng vãng lai'], data['totalQuantityOddCustomer']],

            ];
            chartGrowthCustomer(dataCustomer);

            //Biểu đồ nhóm thẻ dịch vụ.
            chartGrowthServiceCardGroup(data['dataServiceCardGroup']);
...
$('#service-card').select2().on('select2:select', function () {
    filter();
});
// Chọn ngày.
...
function filter() {
    var time = $('#time').val();
    var serviceCard = $('#service-card').val();
