...
function index() {
    $.ajax({
        url: laroute.route('admin.report-revenue.service-card.index'),
        method: "POST",
        data: {time: $('#time').val()},
        dataType: "JSON",
...
function filter() {
    ...
    var numberServiceCard = $('#number-service-card').val();
    if (time == '' && branch == '' && numberServiceCard == '') {
        index()
    } else {
        $.ajax({
...
$('#number-service-card').select2().on('select2:select', function () {
    filter();
});
