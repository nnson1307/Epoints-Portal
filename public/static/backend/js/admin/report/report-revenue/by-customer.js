...
//Biểu đồ.
function chart(listCustomer, listValue) {
...
function index() {
    $.ajax({
        url: laroute.route('admin.report-revenue.customer.index'),
        method: "POST",
        data: {time: $('#time').val()},
        dataType: "JSON",
        success: function (data) {

            $('#container').height(data['countListCustomer'] * 50);
            chart(data['list'], data['seriesData']);
            // if (data['list'].length > 10 && data['seriesData'].length > 10) {
            //     let list = new Array();
            ...
            //
            //     for (let j = 0; j < 9; j++) {
            //         seriesData.push(data['seriesData'][j]);
            //     }
            //     let other = 0;
            //     for (let z = 9; z < data['seriesData'].length; z++) {
            //         other += data['seriesData'][z];
            //     }
            //     seriesData.push(other);
            //     chart(list, seriesData);
            // } else {
            //     chart(data['list'], data['seriesData']);
            // }
            $.getJSON(laroute.route('translate'), function (json) {
...
$('#number-customer').select2().on('select2:select', function () {
    filter();
});
...
function filter() {
    ...
    var numberCustomer = $('#number-customer').val();
    var customer = '';
    if (time == '' && branch == '' && numberCustomer == '') {
        index()
    } else {
        $.ajax({
