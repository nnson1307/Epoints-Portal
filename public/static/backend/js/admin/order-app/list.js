...
var order = {
        ...
        $(document).ready(function () {
            ...
            $('#search').bind("keyup", function () {
                ...
                mApp.block("#m_blockui_1_content", {
                    ...
                    type: "loader",
                    state: "success",
                    message: order.jsontranslate["Đang tải..."]
                });
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.type != 'member_card') {
                            ...
                            $.map(response.list, function (a) {
                                ...
                                if (a.count_using != 0) {
                                    if (a.count_using == order.jsontranslate['Không giới hạn']) {
                                        tpl = tpl.replace(/{quantity}/g, 'KGH');
                                    } else {
                                        tpl = tpl.replace(/{quantity}/g, order.jsontranslate['Còn '] + a.count_using);
                                    }

                                    tpl = tpl.replace(/{quantity_app}/g, a.count_using);
                                } else {
                                    tpl = tpl.replace(/{quantity}/g, order.jsontranslate['Không giới hạn']);
                                    tpl = tpl.replace(/{quantity_app}/g, order.jsontranslate['Không giới hạn']);
                                }
                                $('.append').append(tpl);
                                $.each($('#table_add tbody tr'), function () {
                                    ...
                                    if (value_code == code) {
                                        ...
                                        if (quantity_card != 'Không giới hạn') {
                                            ...
                                            $('.card_check_' + a.customer_service_card_id + '').find('.quantity').append(order.jsontranslate['Còn '] + $('.card_check_' + a.customer_service_card_id + '').find('.quantity_card').val() + order.jsontranslate[' (lần)']);
                                        }
                                    }
                                });
                            });
            ...
            $('.quantity, .price').change(function () {
                ...
                $(this).closest('.tr_table').find('.amount-tr').append(formatNumber(amount.toFixed(decimal_number)) + order.jsontranslate['đ']);
            ...
            $('.remove_card').click(function () {
                ...
                $('.card_check_' + id + '').find('.quantity').append(order.jsontranslate['Còn '] + $('.card_check_' + id + '').find('.quantity_card').val() + order.jsontranslate['(lần)']);
                $(this).closest('.tr_table').remove();
            ...
            $('#receipt_type').select2({
                placeholder: order.jsontranslate['Chọn hình thức thanh toán']
                ...
                if (methodId == 'MEMBER_MONEY') {
                    let money = $('#money').val();
                    tpl = tpl.replace(/{money}/g, order.jsontranslate['(Còn '] + formatNumber(money) + ')');
                } else {
            ...
            $('.amount').change(function () {
                ...
                $('.append_bill').append(formatNumber(sum.toFixed(decimal_number)) + order.jsontranslate['đ']);
                ...
                $('.amount_bill').append(formatNumber(amount_bill.toFixed(decimal_number)) + order.jsontranslate['đ']);
            ...
            $('.quantity_card').change(function () {
                ...
                var quan_db = $(this).closest('.tr_table').find("input[name='quantity_hidden']").val();

                if (quan_val <= quan_db) {
                    var id = $(this).closest('.tr_table').find("input[name='id']").val();
                    $('.card_check_' + id + '').find('.quantity_card').val(quan_db - quan_val);
                    $('.card_check_' + id + '').find('.quantity').empty();
                    $('.card_check_' + id + '').find('.quantity').append(order.jsontranslate['Còn '] + $('.card_check_' + id + '').find('.quantity_card').val() + ' (lần)');
                } else {
                    if (quan_db != 0) {
                        $(this).val(quan_db);
                    }
    ...
    chooseType: function (objectType) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                tpl = tpl.replace(/{category_name}/g, order.jsontranslate['Tất cả'])
    ...
    append_table: function (id, price, type, name, code, isSurcharge, inventoryManagement, isObjectAttach) {
        ...
        if (check == true) {
            ...
            if (type == 'service') {
                tpl = tpl.replace(/{type}/g, order.jsontranslate['Dịch vụ']);
                tpl = tpl.replace(/{id_type}/g, '1');
            }
            if (type == 'service_card') {
                tpl = tpl.replace(/{type}/g, order.jsontranslate['Thẻ dịch vụ']);
                tpl = tpl.replace(/{id_type}/g, '2');
            }
            if (type == 'product') {
                tpl = tpl.replace(/{type}/g, order.jsontranslate['Sản phẩm']);
            ...
            $('.staff').select2({
                placeholder: order.jsontranslate['Chọn nhân viên'],
            });
            ...
            $('.quantity, .price').change(function () {
                ...
                $(".discount-tr-" + type + "-" + stt + "").append('<input type="hidden" name="discount" value="0">');
                $(".discount-tr-" + type + "-" + stt + "").append('<input type="hidden" name="voucher_code" value="">');
                $(".discount-tr-" + type + "-" + stt + "").append('<a class="abc m-btn m-btn--pill m-btn--hover-brand-od btn btn-sm btn-secondary btn-sm-cus" href="javascript:void(0)" onclick="list.modal_discount(' + amount.toFixed(decimal_number) + ',' + id + ',' + id_type + ',' + stt + ')"><i class="la la-plus icon-sz"></i></a>');
                //End update
                ...
                $('.discount_bill').empty();
                $('.discount_bill').append(0 + order.jsontranslate['đ']);
                ...
                if (discount != 0) {
                    $(this).closest('.tr_table').find('.discount-tr-' + type + '-' + stt + '').prepend('<a class="abc" href="javascript:void(0)" onclick="list.close_amount_add(' + id + ',' + id_type + ',' + stt + ')"><i class="la la-close cl_amount"></i></a>');
                } else {
                    $(this).closest('.tr_table').find('.discount-tr-' + type + '-' + stt + '').prepend('<a class="abc btn ss--button-cms-piospa m-btn m-btn--icon m-btn--icon-only" href="javascript:void(0)" onclick="list.modal_discount_add(' + amount.toFixed(decimal_number) + ',' + id + ',' + id_type + ',' + stt + ')"><i class="la la-plus icon-sz"></i></a>');
                }
                order.getPromotionGift();
                discountCustomerInput();
            });
    ...
    submit_send_email: function () {
        $('#submit_email').validate({
            ...
            messages: {
                enter_email: {
                    required: order.jsontranslate['Hãy nhập email'],
                    email: order.jsontranslate['Email không hợp lệ']
                },
            },
            submitHandler: function () {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        if (res.success == 1) {
                            swal(order.jsontranslate["Gửi email thành công"], "", "success");
            ...
            success: function (res) {
                ...
                if (res.gift > 0) {
                    $.map(res.arr_gift, function (a) {
                        ...
                        tpl = tpl.replace(/{stt}/g, stt_tr);
                        tpl = tpl.replace(/{name}/g, a.gift_object_name + ' (' + order.jsontranslate['quà tặng'] + ')');
...
var list = {
    remove: function (obj, id) {
        ...
        swal({
            title: order.jsontranslate['Thông báo'],
            text: order.jsontranslate["Bạn có muốn xóa không?"],
            type: 'warning',
            showCancelButton: true,
            confirmButtonText: order.jsontranslate['Xóa'],
            cancelButtonText: order.jsontranslate['Hủy'],
            onClose: function () {
            ...
            if (result.value) {
                $.post(laroute.route('admin.order.remove', {id: id}), function () {
                    swal(
                        order.jsontranslate['Xóa thành công'],
                        '',
                        'success'
                    );
    ...
    apply_branch: function (id) {
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#branch_id').select2({
                    placeholder: order.jsontranslate['Chọn chi nhánh']
                });
            }
        });
    },
    submit_apply_branch: function (id) {
        ...
        form.validate({
            ...
            messages: {
                branch_id: {
                    required: order.jsontranslate['Hãy chọn chi nhánh']
                }
            },
        });
    ...
    syncOrder: function () {
        ...
        form.validate({
            ...
            messages: {
                number_time: {
                    required: order.jsontranslate['Hãy nhập số giờ đồng bộ'],
                    number: order.jsontranslate['Số giờ đồng bộ không hợp lệ'],
                    min: order.jsontranslate['Số giờ đồng bộ tối thiểu 1']
                },
            },
        });
        ...
        $.ajax({
            url: laroute.route('admin.order-app.sync-order'),
            method: 'POST',
            dataType: 'JSON',
            data: {
                number_time: $('#number_time').val()
            },
            success: function (res) {
                if (res.error == false) {
                    swal(res.message, "", "success").then(function (result) {
            ...
            error: function (res) {
                ...
                swal(order.jsontranslate['Đồng bộ thất bại'], mess_error, "error");
            }
        });
    }
};
