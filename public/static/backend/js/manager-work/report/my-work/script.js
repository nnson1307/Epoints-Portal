...
var MyWork = {
    ...
    getChart: function (){
        $.ajax({
            url: laroute.route('manager-work.report.get-chart-my-work'),
            data: {},
            method: "POST",
            dataType: "JSON",
            success: function(res) {
                if (res.error == false) {
                    $('.list-my-work').append(res.viewMyWork);
                    $('.list-work-support').append(res.viewSupport);
                    if (res.total == 0){
                        $('#report_chart_status').remove();
                        $('#list_status_text_update').html('');
                    } else {
                        if (res.totalWork != 0)
                        {
                            createChart('report_chart_status', myChart(res.label,res.data,res.color));
                        }
                        $('#report_chart_status_text_update').html(res.total);
                        $('#list_status_text_update').html(res.view);
                    }
                    if (res.total1 == 0){
                        $('#report_chart_status_support').remove();
                        $('#list_status_text_update_support').html('');
                    } else {
                        if (res.totalWork1 != 0)
                        {
                            createChart('report_chart_status_support', myChart(res.label1,res.data1,res.color1));
                        }
                        $('#report_chart_status_text_update_support').html(res.total1);
                        $('#list_status_text_update_support').html(res.view1);
                    }
                } else {
    ...
    getList : function () {
        $.ajax({
            url: laroute.route('manager-work.report.get-list-my-work'),
            data: $('.frmFilter_list_my_work').serialize(),
            method: "POST",
            dataType: "JSON",
            success: function(res) {
                if (res.error == false) {
                    $('.listMyWork').empty();
                    $('.listMyWork').append(res.view);
                } else {
                    swal('',res.message,'error');
                }
            },
