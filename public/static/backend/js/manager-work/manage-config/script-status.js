var ManageConfig = {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    if($('#name').val() == ''){
                        ...
                        $.ajax({
                            url: laroute.route('manager-work.manage-config.add-status-config'),
                            method: 'POST',
                            data: {
                                groupId : groupId,
                                count : count,
                            ...
                            success: function (res) {
                                if (res.error == false) {
                                    ...
                                    //         value: res.detailStatus.manage_status_id,
                                    //         text: res.detailStatus.manage_status_name
                                    //     }));
    ...
    updateConfigStatus : function () {
        $.ajax({
            url: laroute.route('manager-work.manage-config.update-config-status'),
            ...
            success: function(data) {
                if (data.error == false) {
                    swal(data.message,'','success').then(function () {
                        window.location.href = laroute.route('manager-work.manage-config.status');
                    });
                } else {
    ...
    removeBlock: function (groupId,count,status_id) {
        $.getJSON(laroute.route('translate'), function (json) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: laroute.route('manager-work.manage-config.remove-status-config'),
                        method: 'POST',
                        data: {
                            manage_status_id : status_id
                        },
                        success: function (res) {
                            if (res.error == false) {
    ...
    changeActive:function (idConfig){
        ...
        $.ajax({
            url: laroute.route('manager-work.manage-config.notification.update-active'),
            data: {
                idConfig : idConfig,
                is_active : is_active
            },
    ...
    changeColor: function (manage_status_group_config_id,keyValue){
        var color = $('#btn_'+manage_status_group_config_id+'_'+keyValue).attr('data-current-color');
        $('#input_'+manage_status_group_config_id+'_'+keyValue).val(color);
    }
}