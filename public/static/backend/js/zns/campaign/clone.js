...
var AddCampaign = {
    ...
    chooseCustomer: function () {
        $('#show-list-customer .check_lead:checked').each(function (index, value) {
            ...
            if ($('#list-customer-get-notification-table .customer_id_' + type_customer + customer_id).length == 0) {
                ...
                let td_id = id + generate_input('hidden', 'customer_id[' + type_customer + '][]', 'customer_id_' + type_customer + customer_id, customer_id);
                ...
                tr = [td_id, customer_name, customer_phone, preview, $('#button-delete-customer').html()];
    ...
    save: function () {
        $.ajax({
            url: laroute.route('zns.campaign.clone-action'),
            method: "POST",
            data: $('#form-add').serialize(),
            success: function (res) {
                if (res.status == 1) {
                    swal(
                        'Sao chép dịch thành công',
                        '',
                        'success'
                    );
...
$('[name="zns_template_id"]').change(function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            // $('#message-content').val(res.detail.template_tag);
            $('#content_zns').attr('src', res.detail.preview);
            $('#template_id').val(res.detail.template_id);
            $('#template_price').val(res.detail.price);
            $("#list-customer-get-notification-table .fa.fa-eye").closest('a').attr('href',res.detail.preview);
