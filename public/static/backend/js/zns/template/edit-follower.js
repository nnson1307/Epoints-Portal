...
var follower = {
    editSubmit: function () {
        ...
        $.ajax({
            url: laroute.route('zns.template-follower.edit-submit'),
            data: params,
            method: "POST",
            dataType: "JSON",
...
$('[name="type_template_follower"]').change(function () {
    var zns_tempalte_id = $('[name=zns_template_id]').val();
    var params = {'type_template_follower': $(this).val()};
    params = Object.keys(params)
        .map(key => `${key}=${params[key]}`)
        .join('&');

    window.location.href = laroute.route('zns.template-follower.edit',{id : zns_tempalte_id}) + '?' + params;
});
