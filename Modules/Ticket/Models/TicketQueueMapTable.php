...
class TicketQueueMapTable extends Model
{
    protected $table = "ticket_queue_map";
    protected $primaryKey = "ticket_queue_map_id";

    protected $fillable = ['ticket_queue_map_id', 'ticket_id','ticket_queue_id', 'created_at', 'created_by','updated_at','updated_by'];
    ...
    public function getName(){
        $oSelect= self::select("ticket_queue_map_id","path")->get();
        return ($oSelect->pluck("path","ticket_queue_map_id")->toArray());
    }
    
    public function testCode($code, $id)
    {
        return $this->where('path', $code)->where('ticket_queue_map_id', '<>', $id)->first();
    }

    public function add(array $data)
    {
        $oTicketQueueMapTable = $this->create($data);
        return $oTicketQueueMapTable->ticket_queue_map_id;
    }
