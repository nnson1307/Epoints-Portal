        ...
        <div class="m-portlet__body" id="autotable">
            ...
            <div class="table-content m--padding-top-15">
                <div class="m-portlet__body listTable" style="padding: 0px">
                    @include('referral::ReferralMember.detail-referral-child.list')
                </div>
...
@section('after_script')
    ...
    <script>
        // ------Vertical-tree-view
        $(function () {
            $('.vertical-tree-start ul').hide();
            $('.vertical-tree-start>ul').show();
            $('.vertical-tree-start ul.active').show();
            // $('.vertical-tree-start a').on('click', function (e) {
            //     console.log(e)
            //     // var $this = this;
            //     // var $e = e;
            //     // console.log($this)
            //     // $.post(laroute.route('referral.referral-member.getChild'), {}, function(res){
            //     //     $($this).closest('li').find('.vertical-tree-container:first').html(res.data);
            //     //     setTimeout(function (){
        ...
        function loadChild(obj, id, lv){
            var $this = obj;
            console.log($this)
            $.post(laroute.route('referral.referral-member.getChild'), {referral_member_id : id, lv : lv}, function(res){
                $($this).closest('li').find('.vertical-tree-container:first').html(res.data);
                setTimeout(function (){
                    var children = $($this).find('> ul');
                    if (children.is(":visible")) children.hide('fast').removeClass('active');
