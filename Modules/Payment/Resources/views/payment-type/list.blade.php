...
{{--            <th class="tr_thead_list">@lang('HỆ THỐNG')</th>--}}
        ...
        @if(isset($LIST) && count($LIST) > 0)
            @foreach ($LIST as $key => $item)
                <tr>
                    <td>{{$key + 1}}</td>
                    <td>{{$item['payment_type_name_vi']}}</td>
                    <td>{{$item['payment_type_name_en']}}</td>
                    <td>
                        @if ($item['is_active'] == 1)
                            <span class="m-switch m-switch--icon m-switch--success m-switch--sm">
                                <label style="margin: 0 0 0 10px; padding-top: 4px">
                                    <input type="checkbox" class="manager-btn" checked
                                           onchange="listPaymentType.updateStatus('{{$item['payment_type_id']}}', 0)">
                                    <span></span>
                                </label>
                            </span>
                        @else
                            <span class="m-switch m-switch--icon m-switch--success m-switch--sm">
                                <label style="margin: 0 0 0 10px; padding-top: 4px">
                                    <input type="checkbox" class="manager-btn"
                                           onchange="listPaymentType.updateStatus('{{$item['payment_type_id']}}', 1)">
                                    <span></span>
                                </label>
                            </span>
                        @endif
...
{{--                    </td>--}}
                    ...
                    <td>
                        {{--                        @if ($item['is_system'] == 0)--}}
                        <a href="{{route('payment-type.edit', $item['payment_type_id'])}}"
                        ...
                        <a href="javascript:void(0)"
                           onclick="listPaymentType.delete('{{$item['payment_type_id']}}')"
                           class="m-portlet__nav-link btn m-btn m-btn--hover-primary m-btn--icon m-btn--icon-only m-btn--pill"
                           title="@lang('Xóa')">
