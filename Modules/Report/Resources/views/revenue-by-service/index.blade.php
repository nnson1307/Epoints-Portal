    ...
    <div class="m-portlet">
        <div class="m-portlet__head border-bottom-0">
            ...
            <div class="m-portlet__head-tools">
                @if(in_array('admin.report-revenue.service.export-total', session()->get('routeList')))
                    <form action="{{route('admin.report-revenue.service.export-total')}}" method="POST">
                        {{ csrf_field() }}
                        ...
                        <input type="hidden" id="export_service_id_total" name="export_service_id_total">
                ...
                @endif
                @if(in_array('admin.report-revenue.service.export-detail', session()->get('routeList')))
                    <form action="{{route('admin.report-revenue.service.export-detail')}}" method="POST">
                        {{ csrf_field() }}
                        ...
                        <input type="hidden" id="export_service_id_detail" name="export_service_id_detail">
                        <button type="submit"
                                class="btn btn-info btn-sm m-btn m-btn--icon m-btn--pill color_button m--margin-right-10">
                                        <span>
                                            <i class="la la-files-o"></i>
        ...
        <div class="m-portlet__head">
            <ul class="nav nav-tabs m-0 align-items-center">
                <li class="nav-item">
                    <a class="nav-link"
                       href="{{route('admin.report-revenue.service-group')}}">{{__('Theo nhóm dịch vụ')}}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link active"
                       href="{{route('admin.report-revenue.service')}}">{{__('Theo dịch vụ')}}</a>
                </li>
            </ul>
        </div>
        <div class="m-portlet__body">
            <div class="m-form m-form__group m-form--label-align-right">
                <div class="row">
                    ...
                    <div class="row col-lg-9 align-conter1 ss--text-white form-group">
                        ...
                        <div class="row col-lg-12 form-group">
                            <div class="col-lg-6 form-group">
                                <select name="service_category_id" style="width: 100%" id="service_category_id"
                                        class="form-control m_selectpicker">
                                    <option value="">{{__('Chọn nhóm dịch vụ')}}</option>
                                    @foreach($optionServiceCategories as $key => $value)
                                        <option value="{{$value['service_category_id']}}">{{$value['name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-6 form-group">
                                <select name="service_id" style="width: 100%" id="service_id"
                                        class="form-control m_selectpicker">--}}
            ...
            <div class="form-group m-form__group m--margin-top-10">
                ...
                <div id="autotable">
                    <form class="frmFilter">
                        ...
                        <input type="hidden" id="service_id_detail" name="service_id_detail">
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/report/revenue/by-service/script.js?v='.time())}}"
            type="text/javascript"></script>
    <script>
        revenueByService._init();
    </script>
@stop
