            ...
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    ...
                    <h3 class="m-portlet__head-text">
                        {{__('THỐNG KÊ THẺ DỊCH VỤ')}}
                    </h3>
                </div>
            </div>

            <div class="m-portlet__head-tools">
                @if(in_array('admin.report-growth.service-card.export-total', session()->get('routeList')))
                    <form action="{{route('admin.report-growth.service-card.export-total')}}" method="POST">
                        {{ csrf_field() }}
                        <input type="hidden" id="export_time_total" name="export_time_total">
                        <input type="hidden" id="export_service_card_total" name="export_service_card_total">
                ...
                @endif
                @if(in_array('admin.report-growth.service-card.export-detail', session()->get('routeList')))
                    <form action="{{route('admin.report-growth.service-card.export-detail')}}" method="POST">
                        {{ csrf_field() }}
                        <input type="hidden" id="export_time_detail" name="export_time_detail">
                        <input type="hidden" id="export_service_card_detail" name="export_service_card_detail">
                        <button type="submit"
                                class="btn btn-info btn-sm m-btn m-btn--icon m-btn--pill color_button m--margin-right-10">
                                        <span>
                                            <i class="la la-files-o"></i>
        ...
        <div class="m-portlet__body">
            <div class="m-form m-form__group m-form--label-align-right">
                <div class="row">
                    ...
                    <div class="col-lg-3 ss--col-xl-4 ss--col-lg-12 form-group">
                        <select name="service-card" style="width: 100%" id="service_card"
                                class="form-control m_selectpicker">--}}
                            <option value="">{{__('Tất cả thẻ dịch vụ')}}</option>
                            @foreach($optionServiceCard as $value)
            ...
            <div class="form-group m-form__group m--margin-top-10">
                ...
                <div id="autotable" class="pt-4">
                    <form class="frmFilter">
                        ...
                        <input type="hidden" id="service_card_detail" name="service_card_detail">
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/report/statistics/by-service-card/script.js')}}"
            type="text/javascript"></script>
    <script>
        statisticServiceCard._init();
    </script>
@stop