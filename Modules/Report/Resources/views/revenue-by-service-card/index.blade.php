            ...
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    ...
                    <h3 class="m-portlet__head-text">
                        {{__('BÁO CÁO DOANH THU THEO THẺ DỊCH VỤ')}}
                    </h3>
                </div>
            </div>

            <div class="m-portlet__head-tools">
                @if(in_array('admin.report-revenue.service-card.export-total', session()->get('routeList')))
                    <form action="{{route('admin.report-revenue.service-card.export-total')}}" method="POST">
                        {{ csrf_field() }}
                        ...
                        <input type="hidden" id="export_number_service_card_total" name="export_number_service_card_total">
                        <input type="hidden" id="export_service_card_id_total" name="export_service_card_id_total">
                ...
                @endif
                @if(in_array('admin.report-revenue.service-card.export-detail', session()->get('routeList')))
                    <form action="{{route('admin.report-revenue.service-card.export-detail')}}" method="POST">
                        {{ csrf_field() }}
                        ...
                        <input type="hidden" id="export_number_service_card_detail" name="export_number_service_card_detail">
                        <input type="hidden" id="export_service_card_id_detail" name="export_service_card_id_detail">
                        <button type="submit"
                                class="btn btn-info btn-sm m-btn m-btn--icon m-btn--pill color_button m--margin-right-10">
                                        <span>
                                            <i class="la la-files-o"></i>
            ...
            <div class="m-form m-form__group m-form--label-align-right">
                <div class="row">
                    ...
                    <div class="col-lg-9 row align-conter1 ss--text-white form-group">
                        ...
                        <div class="col-lg-12 row align-conter1 ss--text-white form-group">
                            <div class="col-lg-6 form-group">
                                <select name="number_service_card" style="width: 100%" id="number_service_card"
                                        class="form-control m_selectpicker">--}}
                                    <option value="">{{__('Giới hạn thẻ dịch vụ')}}</option>
                            ...
                            <div class="col-lg-6 form-group">
                                <select name="service_card_id" style="width: 100%" id="service_card_id"
                                        class="form-control m_selectpicker">--}}
                                    <option value="">{{__('Chọn thẻ dịch vụ')}}</option>
                                    @foreach($serviceCard as $key => $value)
                                        <option value="{{$value['service_card_id']}}">{{$value['name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            <div class="form-group m-form__group m--margin-top-10">
                ...
                <div id="autotable">
                    <form class="frmFilter">
                        ...
                        <input type="hidden" id="number_service_card_detail" name="number_service_card_detail">
                        <input type="hidden" id="service_card_id_detail" name="service_card_id_detail">
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/report/revenue/by-service-card/script.js')}}"
            type="text/javascript"></script>
    <script>
        revenueByServiceCard._init();
    </script>
@stop
