...
//        unset($filter['search']);
    ...
    protected function _listServicesCarStill($filter){
        $param = [
        ...
        $ds = $this
            ->select('services.service_id as service_id', 'services.service_name as service_name', 'services.service_code as service_code',
                'services.price_standard','services.price_standard as new_price')
            ->where('services.is_deleted', 0)
            ->where("services.is_actived", 1)
            ->where("services.is_surcharge", 0)
            ->whereNotIn('services.service_id', function ($ds) use($timeCheck) {
                $ds->select('customer_appointment_details.object_id')
                    ->from('customer_appointment_details')
                    ->join(
    ...
    public function listServicesCarStill($filter){
        $select = $this->_listServicesCarStill($filter);
