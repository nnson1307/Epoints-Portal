...
{{--                                                    <option value="">@lang('Chọn trạng thái')</option>--}}
                                    ...
                                    <div class="col-12">
                                        <div class="form-group m-form__group">
                                            ...
                                            <div class="input-group">
{{--                                                <input type="hidden" name="customer_deal_id" value="{{$detailDeal['customer_id_join']}}">--}}
                                                <input type="hidden" name="customer_deal_id" value="{{$detailDeal['deal_id']}}">
{{--                                                <input type="hidden" name="manage_work_customer_type" value="{{$detailDeal['type_customer'] == 'lead' ? 'deal' : $detailDeal['type_customer']}}">--}}
                                                <input type="hidden" name="manage_work_customer_type" value="deal">
                                                <input type="hidden" name="obj_id" value="{{$detailDeal['deal_id']}}">
                                                <select disabled class="form-control select2 select2-active w-100">
                                                    <option value="">{{$detailDeal['customer_full_join']}}</option>
{{--                                                    @foreach ($listCustomer as $item)--}}
{{--                                                        <option value="{{ $item['customer_lead_id'] }}" {{$customer_lead_id == $item['customer_lead_id'] ? 'selected' : ''}}>{{ $item['full_name'] }}</option>--}}
{{--                                                    @endforeach--}}
                                                </select>
                                            </div>
                                        </div>
                ...
                @if ($detailWork == null )
                    <div class="row">
                        <form id="form-search-support">
                            ...
                            <input type="hidden" name="customer_id" value="{{$deal_id}}">
                            <input type="hidden" name="manage_work_customer_type" value="deal">
...
{{--                    <button onclick="listDeal.closeModalCare()"--}}
...
{{--						</span>--}}
{{--                    </button>--}}
{{--                    <button type="submit" onclick="listDeal.submitDealCare('{{$deal_id}}')"--}}
...
{{--                    </button>--}}
                    ...
                    @if ($detailWork != null)
                        <button onclick="listDeal.closeModalCareEdit()"
                                class="btn btn-metal bold-huy m-btn m-btn--icon m-btn--wide m-btn--md">
                            <span>
                            <i class="la la-arrow-left"></i>
                            <span>{{__('HỦY')}}</span>
                    ...
                    @if($detailWork == null)
                        <button type="submit" onclick="listDeal.submitDealCare('{{$deal_id}}')"
                                class="btn btn-info  color_button son-mb  m-btn m-btn--icon m-btn--wide m-btn--md m--margin-left-10">
                            @else
                                <button type="submit" onclick="listDeal.submitDealCareEdit('{{$deal_id}}')"
                                        class="btn btn-info  color_button son-mb  m-btn m-btn--icon m-btn--wide m-btn--md m--margin-left-10">
                                    @endif
                                    <span>
							<i class="la la-check"></i>
