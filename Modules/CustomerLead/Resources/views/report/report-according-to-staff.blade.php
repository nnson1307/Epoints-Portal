                ...
                <td>
                    @if($item > 0)
                        <a href="javascript:void(0)" class="m-card-user__name line-name font-name"
                           onclick="lead.renderPopupReportStaff('no_staff','{{(count($listJourney) > $key ? $listJourney[$key]['journey_code'] : '')}}');">
                            {{$item}}
                        </a>
                    @else
                        {{$item}}
                    ...
                    @foreach($quantity[$key] as $key2 => $item)
                        <td>
                            @if($quantity[$key][$key2] > 0)
                                <a href="javascript:void(0)" class="m-card-user__name line-name font-name"
                                   onclick="lead.renderPopupReportStaff('{{$listStaff[$key]['staff_id']}}','{{(count($listJourney) > $key2 ? $listJourney[$key2]['journey_code'] : '')}}');">
                                    {{$quantity[$key][$key2] }}
                                </a>
                            @else
                                {{$quantity[$key][$key2] }}
