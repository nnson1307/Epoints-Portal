<table class="table table-bordered">
    ...
    <tr class="text-center table-primary">
        <th scope="col" class="text-right">@lang('Trạng thái')</th>
    ...
    <tr class="text-center">
        <th scope="col" class="text-left">@lang('Nguồn')</th>
        @if(isset($listJourney) && count($listJourney) > 0)
            @foreach($listJourney as $item)
            ...
            @foreach($quantity['sumColumn'] as $key => $item)
                <td>
                    ...
                    <a href="javascript:void(0)" class="m-card-user__name line-name font-name"
                       onclick="lead.renderPopupReportCS('','{{(count($listJourney) > $key ? $listJourney[$key]['journey_code'] : '')}}');">
                        {{$item}}
                    </a>
                    @else
                        {{$item}}
                    ...
                    @foreach($quantity[$key] as $key2 => $item)
                        <td>
                            @if($quantity[$key][$key2] > 0)
                                <a href="javascript:void(0)" class="m-card-user__name line-name font-name"
                                   onclick="lead.renderPopupReportCS('{{$listCustomerSource[$key]['customer_source_id']}}','{{(count($listJourney) > $key2 ? $listJourney[$key2]['journey_code'] : '')}}');">
                                    {{$quantity[$key][$key2] }}
                                </a>
                            @else
                                {{$quantity[$key][$key2] }}
