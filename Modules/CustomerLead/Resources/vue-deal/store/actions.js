...
import { getSearchOptionApi, getListCustomerLeadApi } from "@CustomerDeal/utils/api";
...
export default {
  ...
  async getListCustomerDealAction({ commit }, payload) {
    commit("SET_LOADING_STATE", true)
    
    const response = await getListCustomerLeadApi(payload);

    if(response.data){
      commit("SET_INITIAL_DATA", response.data.customerDeal)
