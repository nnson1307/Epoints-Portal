...
import { updateJourneyApi } from "@CustomerLead/utils/api";
...
export default {
    ...
    methods: {
        ...
        async onChanged($event) {
            if($event.added){
                const customerLeadId = $event.added?.element?.customer_lead_id;
                ...
                const updateStatus = await updateJourneyApi({
                    ...
                    pipeline_id: this.currentFilter.pipeline_id
                });
