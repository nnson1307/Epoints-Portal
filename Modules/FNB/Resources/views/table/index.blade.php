    ...
    <div class="m-portlet" id="autotable">
        <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
                ...
                <a href="javascript:void(0)" onclick="configColumn.showPopupConfigTable()"
                   class="btn ss--button-cms-piospa m-btn m-btn--icon m-btn--pill btn_add_pc btn-sm mr-3">
                    <span>
                        <i class="fa fa-plus-circle"></i>
                ...
                <a href="javascript:void(0)" onclick="Table.showPopupTable()"
                   class="btn ss--button-cms-piospa m-btn m-btn--icon m-btn--pill btn_add_pc btn-sm">
                    <span>
                        <i class="fa fa-plus-circle"></i>
                        <span> {{ __('Thêm bàn') }}</span>
                    </span>
                </a>
                <a href="{{route('fnb.table')}}" data-toggle="modal" data-target="#modalAdd"
        ...
        <div class="m-portlet__body">
            <div class="menu-bar">
                <div class="areas">
                    <a href="{{route('fnb.areas')}}" class="btn btn-info menu-button table" style="color:black;font-size: 20px">
                        {{ __('Khu vực') }}
                    </a>
                </div>
                <div class="menu">
            ...
            <form id="frm-search" class="frmFilter bg clear-form" autocomplete="OFF">
                <div class="row padding_row">
                            ...
                            @elseif($item['column_type'] == 'select2')
                                <select class="form-control {{$item['column_class']}}" id="{{$item['column_id']}}" name="{{$item['column_name']}}">
                                    ...
                                    @elseif($item['column_name'] == 'branch_id')
                                        ...
                                        @endforeach
                                    @elseif($item['column_name'] == 'area_id')
                                        @foreach($listAreas as $itemValue)
                                            <option value="{{$itemValue['area_id']}}">{{$itemValue['area_name']}}</option>
                                        @endforeach
                                    @elseif($item['column_name'] == 'created_name')
                                        @foreach($listStaff as $itemValue)
                                            <option value="{{$itemValue['staff_id']}}">{{$itemValue['full_name']}}</option>
                                        @endforeach
                                    @elseif($item['column_name'] == 'updated_name')
                                        @foreach($listStaff as $itemValue)
                                            <option value="{{$itemValue['staff_id']}}">{{$itemValue['full_name']}}</option>
                                        @endforeach
                    ...
                    <div class="col-lg-3" style="padding-bottom: 10px;">
                        <a href="{{route('fnb.table')}}" class="btn btn-refresh ss--button-cms-piospa m-btn--icon mr-3">
                            {{ __('Xóa bộ lọc') }}
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/fnb/table/script.js?v='.time())}}"></script>
    <script>
        table._init();
    </script>
