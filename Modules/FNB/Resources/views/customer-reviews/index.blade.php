    ...
    <div class="m-portlet" id="autotable">
        ...
        <div class="m-portlet__body">
            ...
            <form class="frmFilter ss--background search-policy">
                <div class="row ss--bao-filter">
                    ...
                    <div class="col-lg-3">
                        <div class="form-group m-form__group">
                            <div class="input-group">
                                <select class="form-control" id="status" name="review_list_id">
                                    <option value="">{{__('Chọn loại đánh giá')}}</option>
                                    @foreach($reviewList as $key => $val)
                                        <option value="{{$val['review_list_id']}}" {{isset($input['review_list_id']) && $input['review_list_id'] == $val['review_list_id'] ? 'selected' : ''}}>{{$val['review_name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="m-portlet__body" style="padding-right: 15px;padding-top: 15px;padding-bottom: 12px">
                    <div class="text-right">
                        <a href="{{route('fnb.customer-review')}}"
            ...
            <div class="table-content table-content-font-a mt-3">
                @include('fnb::customer-reviews.list')
            </div>
            <!-- end table-content -->
        </div>
    </div>
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/fnb/customer-reviews/script.js?v='.time())}}"></script>
    <script>
        customerReviews._init();
    </script>
@stop
