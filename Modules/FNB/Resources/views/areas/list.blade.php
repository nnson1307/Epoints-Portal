        ...
        @if(isset($list) && count($list) != 0)
            @foreach ($list as $key => $item)
                <tr>
                    ...
                    <td>
                        <button onclick="areas.showPopup({{$item}})" type="button" class="m-portlet__nav-link btn m-btn m-btn--hover-primary m-btn--icon m-btn--icon-only m-btn--pill"
                            title="{{__('Chỉnh sửa')}}" >
                            <i class="la la-edit"></i>
                        </button>
                        <button onclick="areas.deleteAreas('{{$item['area_id']}}','{{$item['area_code']}}')" type="button"
