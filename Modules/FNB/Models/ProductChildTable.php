...
class ProductChildTable extends Model
{
    ...
    protected $fillable
        = [
            ...
            'product_code', 'product_child_name','product_child_name_en',
            'unit_id', 'cost', 'price', 'created_at', 'updated_at',
            'created_by', 'updated_by', 'is_deleted', 'is_actived', 'slug','slug_en',
            'is_sales', 'type_app', 'percent_sale', 'inventory_management',
            'is_display', 'product_child_sku','product_attribute_json'
        ];
    ...
    public function _getList(&$filters = [])
    {
        ...
        if (isset($filters['product_child_name'])){
            $oSelect = $oSelect->where('product_child_name','like','%'.$filters['product_child_name'].'%');
            unset($filters['product_child_name']);
        }
    ...
    */
    ...
    /*
     * get item master theo product
     */
    public function getItemMaster($productId)
    {
        return $this
            ->leftJoin('products', 'products.product_id', '=', 'product_childs.product_id')
            ->leftJoin('units', 'units.unit_id', '=', 'product_childs.unit_id')
            ->select(
                ...
                'products.staff_commission_value',
                'product_childs.is_sales',
                'product_childs.product_attribute_json',
                'product_childs.type_app',
                'product_childs.percent_sale',
                'product_childs.is_master',
                "{$this->table}.is_remind",
            ...
            ->where('product_childs.product_id', $productId)
            ->where('product_childs.is_master', self::IS_MASTER)
            ->first();
    }
    ...
    public function getProductChildByProductId($id)
    {
        $select = $this->leftJoin('products', 'products.product_id', '=', 'product_childs.product_id')
            ->select(
                ...
                'product_childs.product_child_sku',
    ...
    public function searchProduct($keyword)
    {
        $select = $this->where('is_deleted', self::IS_NOT_DELETED)
            ...
            ->orWhere('product_child_sku', 'like', '%' . $keyword . '%')
            ->get();
    ...
    public function getList($filter = [])
    {
        ...
        if (isset($filter['keyword']) && $filter['keyword'] != null) {
            $select->where(function ($query) use ($filter) {
                $query->where("product_childs.product_child_name", "LIKE", "%" . $filter["keyword"] . "%")
                    ->orWhere("product_childs.product_code", "LIKE", "%" . $filter["keyword"] . "%")
                    ->orWhere("product_childs.product_child_sku", "LIKE", "%" . $filter["keyword"] . "%");
            });
        }
    ...
    {
        $select = $this
            ->select('product_child_id')
            ->orderBy('product_child_id', 'desc')
            ->limit(1);
        return $select->first();
    }
    ...
    {
        $select = $this->where('product_child_sku', $sku);
        if ($id != null) {
            $select->where('product_child_id', '<>', $id);
        }
        return $select->first();
    }
    ...
    public function getListPagination($filter = []){
        ...
        $oSelect = $this
            ...
            ->where($this->table.'.is_deleted', self::IS_NOT_DELETED)->orderBy($this->primaryKey, 'desc')
            ->orderBy($this->primaryKey, 'desc');

        if (isset($filter['product_child_name'])){
            $oSelect = $oSelect->where($this->table.'.product_child_name','like','%'.$filter['product_child_name'].'%');
        }
        ...
        if (isset($filter['list_product_child_id'])){
            $oSelect = $oSelect->whereNotIn($this->table.'.product_child_id',$filter['list_product_child_id']);
        }

        if (isset($filter['is_topping'])){
            $oSelect = $oSelect->where('products.is_topping', $filter['is_topping']);
        }

        return $oSelect->paginate($display, $columns = ['*'], $pageName = 'page', $page);
    }

    public function getAllByArrChildID($arrChildId){
        return $this->whereIn('product_child_id',$arrChildId)->get();
    }
}