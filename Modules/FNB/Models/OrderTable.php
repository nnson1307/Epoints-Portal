...
class OrderTable extends Model
{
    ...
    public function _getList(&$filter = [])
    {
        ...
        return $ds->whereNotNull('fnb_table_id');
    }
    ...
    public function getItemDetail($id)
    {
        $ds = $this
            ->select(
                ...
                'orders.amount as amount',
                'orders.fnb_table_id as fnb_table_id',
                'orders.fnb_customer_id as fnb_customer_id',
                'orders.process_status as process_status',
                'orders.order_id as order_id',
                ...
                'fnb_table.seats as table_seats',
            ...
            ->leftJoin('staffs', 'staffs.staff_id', '=', 'orders.created_by')
            ->leftJoin('fnb_table','fnb_table.table_id',$this->table.'.fnb_table_id')
    ...
    /**
     * Lấy danh sách đơn hàng theo bàn chưa thanh toán thành công
     */
    public function getListOrderByTable($tableId){
        return $this
            ->select(
                $this->table.'.*',
                'fnb_table.name as table_name'
            ...
            ->leftJoin('fnb_table','fnb_table.table_id',$this->table.'.fnb_table_id')
            ->where($this->table.'.fnb_table_id',$tableId)
            ->where($this->table.'.is_deleted', 0)
            ->whereNotIn($this->table.'.process_status',['paysuccess','ordercancle'])
            ->whereNotNull($this->table.'.fnb_table_id')
            ->get();
    }
    ...
    public function getListOrder($filter = [])
    {
        ...
        $ds = $this
            ->select(
                ...
                "{$this->table}.time_address",
                "{$this->table}.type_shipping",
                DB::raw("(SELECT COUNT(*) FROM order_details where order_details.order_id = {$this->table}.order_id and order_details.order_detail_id_parent is NULL ) as total_product")
            )
            ->leftJoin('customers', 'customers.customer_id', '=', 'orders.customer_id')
            ->leftJoin('customer_groups', 'customer_groups.customer_group_id', '=', 'customers.customer_group_id')
            ->leftJoin('receipts', 'receipts.order_id', '=', 'orders.order_id')
            ...
            ->leftJoin('fnb_table','fnb_table.table_id',$this->table.'.fnb_table_id')
            ->leftJoin('fnb_areas','fnb_areas.area_id','fnb_table.area_id')
            ->whereNotIn($this->table.'.process_status',['ordercomplete','ordercancle','paysuccess'])
            ->where('orders.is_deleted', 0);

        if (isset($filter['order_id'])){
            $ds = $ds->where($this->table.'.order_id','<>',$filter['order_id']);
        }

        if (isset($filter['table_id'])){
            $ds = $ds->where($this->table.'.fnb_table_id',$filter['table_id']);
        }

        if (isset($filter['search'])){
            $ds = $ds->where($this->table.'.order_code','like','%'.$filter['search'].'%');
        }

//        if (!isset($filter['table_id']) && !isset($filter['search'])) {
//            $ds = $ds->where($this->table.'.fnb_table_id','');
//        }
        return $ds->groupBy($this->table.'.order_id')->paginate($display, $columns = ['*'], $pageName = 'page', $page);
    }
