...
//                'product_branch_prices.product_id as product_id',
            ...
            ->where('product_childs.is_deleted', 0)
            ->where('product_childs.is_actived', 1)
//            ->where('product_childs.is_master', 1)
            ->where('products.is_deleted', 0)
            ->where('products.is_actived', 1)
...
//                'product_branch_prices.product_id as product_id',
                ...
                "products.product_name",
                "products.product_id",
                "products.description as description_vi",
...
//            ->where('product_childs.is_master', 1)
    ...
    public function getItemBranchByChildId($branch, $arrProduct)
    {
...
//            ->where('product_childs.is_master', 1)
            ...
            ->where('products.is_actived', 1)
            ->where('products.is_topping', 0)
            ->whereIn('product_childs.product_id',$arrProduct)
...
////            ->where('product_childs.is_master', 1)
        ...
        if (isset($filter['branch_id'])){
            $ds = $ds->where('product_branch_prices.branch_id', $filter['branch_id']);
        }
        if (isset($filter['arr_product_attribute_id'])){
            foreach($filter['arr_product_attribute_id'] as $item){
                $ds = $ds->whereJsonContains('product_childs.product_attribute_json',(int)$item);
            }
        }

        if (isset($filter['product_id'])){
            $ds = $ds->where('product_childs.product_id', $filter['product_id']);
        }
