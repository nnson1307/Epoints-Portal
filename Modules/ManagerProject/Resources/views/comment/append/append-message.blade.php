<tr class="{{isset($data['parent_id']) ? 'tr_child_'.$detail['manage_project_comment_id'] : 'tr_'.$detail['manage_project_comment_id']}}">
    <td>
        ...
        <p class="mb-0">
            @if(\Illuminate\Support\Facades\Session::has('is_staff_work_project') == false || \Illuminate\Support\Facades\Session::get('is_staff_work_project') == 1)
                @if(!isset($data['parent_id']))
                    <a href="javascript:void(0)" class="reply_message" onclick="Comment.showFormChat({{$detail['manage_project_comment_id']}})">{{ __('managerwork::managerwork.answer') }} </a>
                @endif
            @endif
    ...
    <tr>
        ...
        <td>
            <table class="table-message">
                ...
                <tbody class="tr_child_{{$detail['manage_project_comment_id']}}">
                </tbody>
            </table>
        </td>
    </tr>
