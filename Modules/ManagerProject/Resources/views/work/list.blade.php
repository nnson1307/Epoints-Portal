<div class="table-responsive">
    <table class="table table-striped m-table m-table--head-bg-default" id="table-config">
        ...
        <tbody style="font-size: 13px">
            <td colspan="{{count($showColumn)}}">
                <select class="form-control" onchange="ProjectWork.phareChange()" id="manage_project_phase_id" >
                    <option value="">{{__('Chọn giai đoạn')}}</option>
                    @foreach($listPhare as $item)
                        <option value="{{$item['manage_project_phase_id']}}" {{isset($params['manage_project_phase_id']) && $item['manage_project_phase_id'] == $params['manage_project_phase_id'] ? 'selected' : ''}}>{{$item['name']}}</option>
                    @endforeach
                </select>
            </td>
            @if(isset($list))
                    ...
                    @foreach ($showColumn as $column)
                        @if($column['active'] == 1)
                            ...
                            if(isset($column['type']) && $column['type'] == 'link'){
                                $params['link'] =  route('manager-project.work.detail', ['id' => $item['manage_work_id'] , 'manage_project_id' => $params['manage_project_id']]);
                            }
                            ...
                            @endif>
                                @if ($column['type'] == 'function')
                                    ...
                                    @else
                                        @if(in_array(\Auth::id(),$listStaffManage))
                                            <button onclick="WorkChild.showPopup('{{ $item['manage_work_id'] }}')" class="m-portlet__nav-link btn m-btn m-btn--hover-primary m-btn--icon m-btn--icon-only m-btn--pill">
                                                <i class="la la-edit"></i>
                                            </button>
                                            <button onclick="ManagerWork.remove(this, '{{ $item['manage_work_id'] }}','{{ $item['total_child_job'] }}')"
    ...
    {{--                                                </button>--}}
                                ...
                                @else
                                    @include('manager-project::column_config.'.$column['type'], $params)
                                @endif
                            </td>
                        @endif
                    @endforeach
