...
namespace Modules\ManagerProject\Repositories\ManagerDocument;
...
class ManagerDocumentRepository implements ManagerDocumentRepositoryInterface
{
    /**
...
//                }
    ...
    public function searchDocument($data)
    {
        try {
            ...
            $view =  $view = view('manager-project::work.append.append-list-document', ['listDocument' => $listDocument])->render();
            return [
                'error' => false,
                'view' => $view
            ];
        ...
        try{
            ...
            if (isset($data['is_edit_work'])) {
                ...
                $dataWork = [
                    ...
                    'date_end' => isset($data['type_copy']) ? (Carbon::createFromFormat('d/m/Y H:i', $data['date_end'])->format('Y-m-d H:i:00')) : (isset($data['time_end']) ? Carbon::createFromFormat('d/m/Y', $data['date_end'])->format('Y-m-d ' . $data['time_end'] . ':00') : Carbon::createFromFormat('d/m/Y', $data['date_end'])->format('Y-m-d 23:00:00')),
                    'processor_id' => $data['processor_id'],
...
//                    $detailOld = $mManageWork->getDetail($idWork);
    ...
    public function searchWork($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.append.append-list-work-child', ['listWorkChild' => $listWorkChild])->render();
        ...
        if (count($data['list']) > 0) {
            ...
            foreach ($data['list'] as $item) {
                ...
                foreach ($data['listColumn'] as $column) {
                    if ($column['active'] == 1 && !in_array($ic['name'],['Chức năng','Action'])) {
    ...
    public function popupChangeFolder($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.popup.popup-change-folder', ['detail' => $detail, 'access_token' => $access_token,'brandCode' => $brandCode])->render();
