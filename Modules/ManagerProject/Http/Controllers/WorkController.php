...
use Modules\ManagerProject\Http\Requests\Project\ProjectAddRequest;
...
//            $filter['date_end'] = Carbon::parse($param['date_end'])->format('d/m/Y') . ' - ' . Carbon::parse($param['date_end'])->format('d/m/Y');
        ...
        $rProject = app()->get(\Modules\ManagerProject\Repositories\Project\ProjectRepositoryInterface::class);
        $info = $rProject->projectInfoWork($filter['manage_project_id']);
...
//        Danh sách giai đoạn
        ...
        return view('manager-project::work.index', [
            'list' => $list,
    ...
    public function showAddAction()
    {
        return response()->json(
            [
                'html' => view('manager-project::work.add',  $this->loadDetail())->render(),
            ]
        );
    }
    ...
    public function kanbanViewAction(Request $request)
    {
        ...
        if (isset($filters['manage_project_id'])) {
            $rProject = app()->get(ProjectRepositoryInterface::class);
            $mManageProjectStaff = app()->get(ManageProjectStaffTable::class);
            $project = $rProject->getDetailFix($filters['manage_project_id']);
        ...
        return view('manager-project::work.kanban', [
...
//        Danh sách giai đoạn
    ...
    public function editAction(Request $request)
    {
        if ($request->ajax()) {
            ...
            return response()->json(
                [
                    'html' => view('manager-project::work.edit', $jsonString + $this->loadDetail())->render(),
...
//        Kiểm tra công việc có tồn tại
        ...
        if ($detail == null){
            if(isset($param['manage_project_id'])){
                return redirect()->route('manager-project.work',['manage_project_id' => $param['manage_project_id']]);
            } else {
                return redirect()->route('manager-work.project');
            }
        }
...
//        Lấy chi tiết công việc
        ...
        if (isset($detail['manage_project_id'])) {
            $rProject = app()->get(ProjectRepositoryInterface::class);
            $mManageProjectStaff = app()->get(ManageProjectStaffTable::class);
            $project = $rProject->getDetailFix($detail['manage_project_id']);
            $listStaffManage = $mManageProjectStaff->getListAdmin($detail['manage_project_id'],'administration');
            $listStaffProject = $mManageProjectStaff->getListAdmin($detail['manage_project_id']);
            if (count($listStaffManage) != 0){
                $listStaffManage = collect($listStaffManage)->pluck('staff_id')->toArray();
            }
        ...
        $checkStaff = $mManageProjectStaff->checkStaffProject(Auth::id(),$detail['manage_project_id']);
        $detail['is_staff'] = count($checkStaff) != 0 ? 1 : 0;

        if (session()->has('is_staff_work_project')){
            session()->forget('is_staff_work_project');
        }

        session()->put('is_staff_work_project',$detail['is_staff']);

        return view('manager-project::work.detail', [
            'detail' => $detail,
    ...
    public function detailHistoryAction($id)
    {
        ...
        return view('manager-project::work.detail-history', [
            'detail' => $detail,
            'listStaff' => $listStaff
        ] + $this->loadDetail());
    }
    ...
    public function detailDocumentAction($id)
    {
        ...
        return view('manager-project::work.detail-document', [
            'detail' => $detail,
    ...
    public function detailRemindAction($id)
    {
        ...
        return view('manager-project::work.detail-remind', [
            'detail' => $detail,
    ...
    public function detailChildWorkAction($id)
    {
        ...
        return view('manager-project::work.detail-child-work', [
            'detail' => $detail,
...
//            ],
    ...
    {
        $param = $request->all();
        $param['view_popup'] = 'manager-project::work.popup.popup-upload-file-work';
        $data = $this->managerWork->showPopupUploadFile($param);
        return response()->json($data);
    }
    ...
    public function addWork(Request $request){
        ...
        $param = $request->all();
        $data = $this->managerWork->addAndEditWork($param);

        return view('manager-project::work.add-edit-work',$data);
    }
    ...
    public function saveChildWork(Request $request)
    {
        ...
        $data = $rManageWork->saveChildWork($param);
        return response()->json($data);
    }
    ...
    public function loadComment(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($id) {
                $item = $this->managerWork->getItem($id);
                $returnModal = view('manager-project::work.popup.comment-popup', [
    ...
    public function loadFormUpdateProcess(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($id) {
                $item = $this->managerWork->getItem($id);
                $returnModal = view('manager-project::work.popup.update_process', [
    ...
    public function loadFormUpdateDateEnd(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($id) {
                $detail = $this->managerWork->getDetail($id);
                $returnModal = view('manager-project::work.popup.update_date_end', [
                    'detail' => $detail,
                    'manage_work_id' => $id
                ])->render();
                return response()->json(['error' => 0, 'data' => $returnModal]);
    ...
    /**
     * Hiển thị popup chọn nhân viên hỗ trợ
     * @param Request $request
     */
    public function showPopStaffSupportAction(Request $request){
        $param = $request->all();
        $data = $this->managerWork->showPopStaffSupportAction($param);
        return response()->json($data);
    }
    ...
    public function listStaffSupportAction(Request $request)
    {
        ...
        return view('manager-project::work.popup.list-staff-support', $list);
    }
