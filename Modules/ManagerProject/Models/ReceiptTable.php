...
protected $casts  = [
    ...
    public function getListReceipt($filter = [],$param =[]){
        $mSelect = $this
            ->select(
                ...
                "{$this->table}.status",
                "{$this->table}.object_accounting_name as accounting_name",
            ...
            ->leftJoin("receipt_details", "{$this->table}.receipt_id","receipt_details.receipt_id")
            ->leftJoin("payment_method", "receipt_details.payment_method_code","payment_method.payment_method_code");
        ...
        if(isset($filter['obj_id']) && $filter['obj_id'] != null && $filter['obj_id'] != []){
            $mSelect->where("{$this->table}.receipt_id",$filter['obj_id']);
        }
        ...
        if(isset($param['search']) && $param['search'] != null && $param['search'] != []){
            $mSelect->where("{$this->table}.receipt_code",'like','%'.$param['search'].'%');
        }
        ...
        if(isset($param['branch_id']) && $param['branch_id'] != null && $param['branch_id'] != []){
            $mSelect->where("{$this->table}.branch_id",$param['branch_id']);
        }
    ...
    public function editReceipt($data,$id){
        return $this
            ->where('receipt_id',$id)
            ->update($data);
    }
