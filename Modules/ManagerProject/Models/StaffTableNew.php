...
class StaffTableNew extends Model
{
    ...
    public function _getList(&$filter = [])
    {
        ...
        //Filter theo danh sách nhân viên có trong dự án
        if (isset($filter['arr_staff_project']) && !in_array(-1,$filter['arr_staff_project'])) {
            $ds->whereIn("{$this->table}.staff_id", $filter['arr_staff_project']);
        }

        unset($filter['arr_staff_project']);
        unset($filter['manage_project_id']);
