        ...
        @if(isset($LIST) && count($LIST) > 0)
            ...
            @foreach ($LIST as $k => $item)
                ...
                <tr>
                    ...
                    <td>
                        @if($item['is_approve_level1'] === null && $item['is_approve_level2'] === null && $item['is_approve_level3'] === null)
                            <a href="{{route('timeoffdays.edit',$item['time_off_days_id'])}}"
                                    class="test m-portlet__nav-link btn m-btn m-btn--hover-success m-btn--icon m-btn--icon-only m-btn--pill"
                                    title="{{__('Sửa')}}">
                                <i class="la la-edit"></i>
                            </a>
                            <a href="{{route('timeoffdays.remove',$item['time_off_days_id'])}}"
