...
class TimeOffDaysShiftsTable extends Model
{
    ...
    {
        return $this->where("{$this->primaryKey}", $daysId)->update(['is_deleted' => 1]);
    }

    public function checkExist($timeWorkingStaffId, $timeOffTypeId)
    {
        return $this->where("time_off_type_id", $timeOffTypeId)
        ->where("time_working_staff_id", $timeWorkingStaffId)->first();
    }
}