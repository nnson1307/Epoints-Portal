    ...
    public function getLists($data = []){
        ...
        $oSelect = $this
            ->select(
                ...
                'sf_time_working_staffs.time_off_days_id',
            
            )->leftJoin("sf_time_working_staffs", "sf_time_working_staffs.shift_id", "=", "{$this->table}.shift_id")
            ->where("sf_time_working_staffs.staff_id" , '=', Auth()->id());
            
            if ($data['working_day_start'] && $data['working_day_end']) {
                $start = Carbon::createFromFormat('d/m/Y',$data['working_day_start'])->format('Y-m-d');
                $end = Carbon::createFromFormat('d/m/Y',$data['working_day_end'])->format('Y-m-d');
                $oSelect->whereBetween("sf_time_working_staffs.working_day" , [$start . ' 00:00:00', $end .' 23:59:59'] );
            }
