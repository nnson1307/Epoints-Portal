    ...
    <table class="table table-striped m-table ss--header-table">
        <thead>
            <tr class="ss--nowrap">
                ...
                <th class="ss--font-size-th">{{ __('Ngân sách ngày') }}</th>
                <th class="ss--font-size-th">{{ __('Ngày áp dụng') }}</th>
            ...
            @if (isset($data) && $data->isNotEmpty())
                @foreach ($data as $key => $item)
                    <tr class="ss--font-size-13 ss--nowrap">
                        ...
                        <td>{{ Carbon\Carbon::parse($item['effect_time'])->format('d/m/Y') }}</td>
                        <td>{{ $item['created_by'] }}</td>
                        <td>{{ Carbon\Carbon::parse($item['created_at'])->format('d/m/Y H:i') }}</td>
                        <td>
                            @if (in_array('kpi.marketing.budget.day', session('routeList')))
                                @if (Carbon\Carbon::now()->format('d/m/Y') >= Carbon\Carbon::parse($item['effect_time'])->format('d/m/Y')) 
                                    <a href="#" 
                                    ...
                                    <button onclick="BudgetMarketingByDay.remove(this, {{ $item['budget_marketing_id'] }})"
