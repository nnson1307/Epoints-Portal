...
class StoreRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            'wifi_name' => [
                ...
                'max:190',
                'required_if:timekeeping_type,==,wifi',
                'unique:sf_timekeeping_config,wifi_name,'.',timekeeping_config_id,is_deleted,0,timekeeping_type,wifi'
            ],
    ...
    {
        return [
            'wifi_name.required_if' => __('Hãy nhập tên wifi'),
            'wifi_name.max' => __('Tên wifi tối đa 190 kí tự'),
            'wifi_name.unique' => __('Tên wifi đã tồn tại'),
            'wifi_ip.required_if' => __('Hãy nhập địa chỉ ip'),
