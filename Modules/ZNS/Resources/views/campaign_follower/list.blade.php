                    ...
                    <td class="ss--text-center">
                        @if(($item->status == "new"))
                            @if(!($item->is_now == 1 && $item->is_actived == 1))
                                <a href="{{ route('zns.campaign-follower.edit',$item->zalo_campaign_follower_id) }}"
                        ...
                        @if($item->status == "new")
                            @if(!($item->is_now == 1 && $item->is_actived == 1))
                                <a href="javascript:void(0);"
                                   onclick="Campaign.removeAction({{$item->zalo_campaign_follower_id}})"
                                   class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill"
                                   title="{{__('Xóa')}}"><i class="la la-trash"></i>
                                </a>
                            @endif
                        @endif
                        <a href="javascript:void(0);" onclick="Campaign.cloneAction({{$item->zalo_campaign_follower_id}})"
                           class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"
                           title="{{__('sao chép')}}"><i class="fa fa-clone"></i>
                        </a>
                    </td>
                    <td class="ss--font-size-13">
                        <a href="{{ route('zns.campaign-follower.view',$item->zalo_campaign_follower_id) }}"
                           class="text-primary"
                           title="{{ route('zns.campaign-follower.view',$item->zalo_campaign_follower_id) }}">
                            {{ $item->name }}
                        </a>
                    </td>
                    ...
                    <td class="ss--text-center ss--font-size-13">
                        ...
                        @elseif($item->status == "new")
                            @if ($item->is_actived)
                                ...
                                <label class="ss--switch">
                                    <input type="checkbox"
                                           onclick="Campaign.changeStatus(this, '{!! $item->zalo_campaign_follower_id !!}', 'publish')"
                            ...
                            @else
                                <span class="m-switch m-switch--icon m-switch--success m-switch--sm">
                                    <label class="ss--switch">
                                        <input type="checkbox"
                                               onclick="Campaign.changeStatus(this, '{!! $item->zalo_campaign_follower_id !!}', 'unPublish')"
