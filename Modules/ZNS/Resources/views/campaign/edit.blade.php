            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    <div class="form-group m-form__group">
                        <input type="hidden" name="zns_campaign_id" value="{{$item->zns_campaign_id}}">
                        <input id="name" name="name" type="text" class="form-control"
                               placeholder="{{ __('Nhập tên chiến dịch') }}" value="{{$item->name}}">
                        <input type="hidden" name="campaign_type"
                               value="{{ isset($params['campaign_type'])?$params['campaign_type']:'zns' }}">
                    </div>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col-lg-9">
                    <div class="param_list">
                        @php
                            ...
                            $input_params = $item->params ? json_decode($item->params,true):[];
                        @endphp
                        @if($input_params && count($input_params))
                            @foreach($input_params as $key_param => $param)
                                <div class="row  m-3 ml-0">
                                    <div class="col-md-6 col-sm-12">
                                        <span class="p-3 bg-secondary">{{$key_param}}</span>
                                    </div>
                                    <div class="col-md-6 col-sm-12">
                                        @if($list_param[$key_param]['type'] == "NUMBER")
                                            <input type="number" name="params[{{$key_param}}]" value="{{$param}}"
                                                   minlength="{{$list_param[$key_param]['min_length']}}"
                                                   maxlength="{{$list_param[$key_param]['max_length']}}"
                                                   class="form-control">
                                        @elseif($list_param[$key_param]['type'] == "STRING")
                                            <input type="text" name="params[{{$key_param}}]" value="{{$param}}"
                                                   minlength="{{$list_param[$key_param]['min_length']}}"
                                                   maxlength="{{$list_param[$key_param]['max_length']}}"
                                                   class="form-control">
                                        @elseif($list_param[$key_param]['type'] == "DATE")
                                            <div class="m-input-icon m-input-icon--right w-50">
                                                <input readonly
                                                       class="form-control m-input daterange-picker date-time"
                                                       name="params[{{$key_param}}]" value="{{$param}}"
                ...
                <div class="m-scrollable m-scroller ps ps--active-y">
                    <table class="table table-striped m-table ss--header-table"
                        ...
                        <tr class="ss--font-size-th ss--nowrap">
                            ...
                            <th class="">{{ __('NỘI DUNG TIN NHẮN') }}</th>
                            <th></th>
                        </tr>
                        </thead>
                            ...
                            @foreach ($list_customer_send as $key => $customer_info)
                                <tr>
                                    ...
                                    <td style="white-space: normal">
                                        {{ $customer_info->type_customer == 'lead' ? $customer_info->full_name_lead : $customer_info->full_name}}
...
@section('after_script')
    ...
    <script src="{{ asset('static/backend/js/zns/campaign/edit.js?v=' . time()) }}"
