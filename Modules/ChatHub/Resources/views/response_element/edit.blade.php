...
<div class="m-portlet m-portlet--head-sm">
    ...
    <form id="form">
        ...
        <div class="m-portlet__body" id="autotable">
            <input type="hidden" name="response_element_id" class="form-control m-input" id="response_element_id" value="{{$response_element['response_element_id']}}">
            <div class="col-lg-12">
                <div class="form-group m-form__group">
                    <label class="black_title">
                        @lang('chathub::response_element.index.TITLE'):<b class="text-danger">*</b>
                    </label>
                    <input type="text" name="title" class="form-control m-input" id="title" value="{{$response_element['title']}}">
            ...
            <div class="col-lg-12">
                <div class="form-group m-form__group">
                    ...
                    <input type="text" name="subtitle" class="form-control m-input" id="subtitle" value="{{$response_element['subtitle']}}">
                    <span class="error-name"></span>
                </div>
            </div>
            <div class="col-lg-12">
                ...
                <div class="form-group m-form__group m-widget19">
                    <div class="m-widget19__pic">
                        <div class="wrap-imge avatar-temp">
                            @if($response_element['image_url'])
                                <img class="m--bg-metal m-image" id="blah-add"
                                 src="{{$response_element['image_url']}}"
                                 alt="{{__('Hình ảnh')}}"
                                 onclick="document.getElementById('getFile').click()" height="100px">
                                <input type="hidden" name="image_url" id="image_url" value="{{$response_element['image_url']}}">
                                <span class="s-delete-img">
                ...
                <div class="form-group m-form__group">
                    ...
                    <select name="button[]" class="form-control m-input select2" id="response_button" multiple>
                        @if (isset($response_button))
                            ...
                            <option value="{{$item['response_button_id']}}" @foreach($response_element['child'] as $child) @if($child->title==$item['title']) selected @endif @endforeach>{{$item['title']}}</option>
                            @endforeach
                        @endif
                    </select>
                    <span class="error-name"></span>
        ...
        <div class="m-portlet__foot">
            <div class="m-portlet__foot m-portlet__no-border m-portlet__foot--fit">
                <div class="m-form__actions m--align-right">
                    ...
                    <button type="button" class="btn btn-success color_button son-mb  m-btn m-btn--icon m-btn--wide m-btn--md btn-add-close m--margin-left-10" onclick="response_element.Update()">
                        <span>
...
@section('after_script')
    ...
    <script type="text/template" id="image-avatar-temp">
        <img class="m--bg-metal m-image" id="blah-add"
                ...
                alt="{{__('Hình ảnh')}}"
                onclick="document.getElementById('getFile').click()" height="100px">
        <input type="hidden" name="image_url" id="image_url" value="0"> 
        <span class="s-delete-img d-none">
            <span href="javascript:void(0)"
                onclick="deleteAvatar()">
                <i class="la la-close"></i>
