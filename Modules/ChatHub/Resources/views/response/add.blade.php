...
@section('title_header')
    <span class="title_header"><img
                ...
                style="height: 20px;">@lang('Thêm phản hồi')</span>
@endsection
@section("after_style")
@endsection
@section('content')
    <div class="row">
        <div class="col-md-12 ">
            <!-- Default box -->
            <form method="POST" action="{{route('chathub.response.create')}}" accept-charset="UTF-8">
                {{ csrf_field() }}
                <div class="box">

                    <div class="box-header with-border">
                        <h3 class="box-title">@lang('Thêm phản hồi')</h3>
                    </div>
                    @if (count($errors) > 0)
                    ...
                    <div class="box-body row">
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <input type="text" name="response_name" value="" class="form-control">
                        </div>
                        <!-- load the view from the application if it exists, otherwise load the one in the package -->
                        <!-- text input -->
                        <div class="form-group col-sm-12">
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <select name="entities[sub_brand][]" class="form-control select2" multiple>
                                @foreach($arrSubBrand as $subBrand)
                                    <option value="{{$subBrand['sub_brand_id']}}">{{$subBrand['sub_brand_name']}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-sm-12">
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <select name="response_content" class="form-control select2">
                                @foreach($arrResponseContent as $responseContent)
                                    <option value="{{$responseContent['response_content_id']}}">{{$responseContent['title']}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-sm-12">
