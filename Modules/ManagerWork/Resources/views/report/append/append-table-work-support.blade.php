<table class="table table-striped m-table ss--header-table">
    ...
    @if(count($listWorkSuppport) != 0)
        @foreach($listWorkSuppport as $key => $item)
            <tr>
                <td>{{($listWorkSuppport->currentPage() - 1)*$listWorkSuppport->perPage() + $key+1 }}</td>
...
{{ $listWorkSuppport->links('manager-work::report.helpers.paging-table-work-support') }}