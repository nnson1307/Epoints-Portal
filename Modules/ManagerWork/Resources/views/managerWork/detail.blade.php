    ...
    <div class="m-portlet m-portlet--head-sm">
        <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
                ...
                @if(isset($detail['is_approve_id']) && $detail['is_approve_id'] == 1 && $detail['manage_status_id'] == 3 && $detail['approve_id'] == \Auth::id())
                ...
                @else
                    @if(in_array(\Auth::id(),[$detail['processor_id'],$detail['assignor_id']]) && $detail['is_edit'] == 1)
                        <button type="button" onclick="WorkDetail.showPopup('{{$detail['manage_work_id']}}')"
                                class="ss--btn-mobiles btn text-uppercase ss--button-cms-piospa ss--btn m-btn m-btn--icon m-btn--wide m-btn--md m--margin-left-10  m--margin-bottom-5">
                            <span class="ss--text-btn-mobi">
                            <i class="la la-check"></i>
                ...
                @if(in_array(\Auth::id(),[$detail['processor_id'],$detail['assignor_id']]) && $detail['is_deleted'] == 1)
                    ...
                    <button type="button"
                            onclick="ManagerWorkDetail.remove(this, '{{ $detail['manage_work_id'] }}','{{ $detail['total_child_job'] }}',true)"
                    ...
                    <div class="row">
                        ...
                        @if(isset($detail['parent_id']))
                            <div class="col-lg-12 mb-2">
                                <a style="color:#5BAABF;font-size: 15px;" href="{{route('manager-work.detail',['id' => $detail['parent_id']])}}">{{__('Parent task')}} #{{$detail['parent_manage_work_code']}}: </a> {{$detail['parent_manage_work_title']}}
                            </div>
                        @endif
                        <div class="col-lg-12">
                            <p class="info_detail_work"> <span>{{__('Tạo bởi'). ' ' . $detail['createdStaff_name']}}</span>
                                ...
                                @endif
                                <span><i class="far fa-comment-alt"></i> ({{$detail['total_message']}})</span> <span><i class="fas fa-paperclip"></i> ({{$detail['total_attach']}})</span> </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-6">
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <td><a href="{{route('manager-work',['manage_type_work_id' => $detail['manage_type_work_id']])}}" >{{$detail['manage_type_work_name']}}</a></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @foreach($detail['list_tag'] as $key => $item)
                                            @if($key != 0)
                                                <a href="{{route('manager-work',['manage_tag_id' => $item['manage_tag_id']])}}">{{' , '.$item['manage_tag_name']}}</a>
                                            @else
                                                <a href="{{route('manager-work',['manage_tag_id' => $item['manage_tag_id']])}}">{{$item['manage_tag_name']}}</a>
                                            @endif
                                        @endforeach
            ...
            <div class="card">
                <div class="card-header" id="headingTwo">
                    ...
                    <div id="collapseTwo" class="collapse show " aria-labelledby="headingTwo">
                        ...
                        <a href="javascript:voi(0)" class="btn-xem-them-description">{{__('Xem thêm')}} <i
...
{{--    <script src="{{ asset('static/backend/js/manager-work/managerWork/detail-work.js?v=' . time()) }}"--}}
{{--            type="text/javascript"></script>--}}
    <script src="{{ asset('static/backend/js/manager-work/managerWork/detail-tab.js?v=' . time()) }}"
            type="text/javascript"></script>

    <script src="{{asset('static/backend/js/admin/service/autoNumeric.min.js')}}"></script>
    <script src="{{ asset('static/backend/js/manager-work/managerWork/list.js?v=' . time()) }}"
            type="text/javascript"></script>
    <script src="{{asset('static/backend/js/manager-work/managerWork/detail-work-remind.js?v='.time())}}"></script>
    <script src="{{ asset('static/backend/js/manager-work/managerWork/detail-work.js?v=' . time()) }}"
            type="text/javascript"></script>
