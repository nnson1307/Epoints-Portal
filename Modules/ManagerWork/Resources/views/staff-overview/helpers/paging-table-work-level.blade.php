<div class="m-datatable m-datatable--default">
    <div class="m-datatable__pager m-datatable--paging-loaded clearfix">
        ...
        <ul class="m-datatable__pager-nav" style="float: right">
            ...
            @else
                <li><a title="First" class="m-datatable__pager-link m-datatable__pager-link--first" data-page="1" onclick="StaffOverview.tableWorkLevel(1)"><i class="la la-angle-double-left"></i></a></li>
                <li><a title="Previous" class="m-datatable__pager-link m-datatable__pager-link--prev" onclick="StaffOverview.tableWorkLevel({{ $paginator->currentPage() - 1 }})" data-page="{{ $paginator->currentPage() - 1 }}"><i class="la la-angle-left"></i></a></li>
            @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li><a class="m-datatable__pager-link m-datatable__pager-link--active"  onclick="StaffOverview.tableWorkLevel({{ $page }})" data-page="{{ $page }}" title="{{ $page }}">{{ $page }}</a></li>
                        @else
                        	<li><a class="m-datatable__pager-link" onclick="StaffOverview.tableWorkLevel({{ $page }})" data-page="{{ $page }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @else
                <li><a title="Next" class="m-datatable__pager-link m-datatable__pager-link--next" onclick="StaffOverview.tableWorkLevel({{ $paginator->currentPage() + 1 }})" data-page="{{ $paginator->currentPage() + 1 }}"><i class="la la-angle-right"></i></a></li>
            	<li><a title="Last" class="m-datatable__pager-link m-datatable__pager-link--last" onclick="StaffOverview.tableWorkLevel({{ $paginator->lastPage() }})" data-page="{{ $paginator->lastPage() }}"><i class="la la-angle-double-right"></i></a></li>
            @endif
        </ul>
        @endif
