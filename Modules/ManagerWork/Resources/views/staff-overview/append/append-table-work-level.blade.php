<table class="table table-striped m-table ss--header-table">
    ...
    <tr>
        ...
        <th>{{ __('managerwork::managerwork.priority') }}</th>
    ...
    <tbody>
    @if(count($listWorkLevel) != 0)
        @foreach($listWorkLevel as $key => $item)
            <tr>
                <td>
                    {{($listWorkLevel->currentPage() - 1)*$listWorkLevel->perPage() + $key+1 }}
                </td>
                <td>{{$item['manage_type_work_icon'] == null ? $item['manage_type_work_name'] : $item['manage_type_work_icon']}}</td>
                <td>
                    {{$item['priority'] == 1 ? __('Cao') : ($item['priority'] == 2 ? __('Bình thường') : ($item['priority'] == 3 ? __('Thấp') : ''))}}
                </td>
...
{{ $listWorkLevel->links('manager-work::staff-overview.helpers.paging-table-work-level') }}