...
@section('title_header')
    <span class="title_header"><img
                ...
                style="height: 20px;"> {{__('QUẢN LÝ CÔNG VIỆC')}}</span>
@endsection
@section("after_style")
    ...
    <div class="m-portlet" id="autotable">
        <div class="m-portlet__head">
            ...
            <div class="m-portlet__head-tools">
                <a href="{{route('manager-work.manage-config.role-edit')}}"
        ...
        <div class="m-portlet__body p-0">
            <ul class="nav nav-tabs nav-pills mb-3" role="tablist" style="margin-bottom: 0;">
                ...
                <li class="nav-item">
                    <a href="javascript:void(0)" class="nav-link active">{{ __('managerwork::managerwork.role') }}</a>
                </li>
            </ul>
                ...
                <table class="table table-striped m-table ss--header-table ss--nowrap">
                    ...
                    @foreach($listRole as $key => $item)
                        <tr>
                            ...
                            <td>{{$key + 1}}</td>
                            <td>{{$item['name']}}</td>
                            <td class="text-center"><input type="radio" disabled name="role[{{$key}}][check]" value="is_all" {{isset($item['is_all']) && $item['is_all'] == 1 ? 'checked' : ''}}></td>
                            <td class="text-center"><input type="radio" disabled name="role[{{$key}}][check]" value="is_branch" {{isset($item['is_branch']) && $item['is_branch'] == 1 ? 'checked' : ''}}></td>
                            <td class="text-center"><input type="radio" disabled name="role[{{$key}}][check]" value="is_department" {{isset($item['is_department']) && $item['is_department'] == 1 ? 'checked' : ''}}></td>
                            <td class="text-center"><input type="radio" disabled name="role[{{$key}}][check]" value="is_own" {{isset($item['is_own']) && $item['is_own'] == 1 ? 'checked' : ''}}></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
