...
use Modules\ManagerWork\Models\FileMinioConfigTable;
...
use Modules\ManagerWork\Models\ManagerProjectTable;
...
use Modules\ManagerWork\Models\ManageWorkLocationTable;
...
use Modules\ManagerWork\Models\StaffTableNew;
use Modules\ManagerWork\Models\TicketTable;
...
class ManagerWorkRepository implements ManagerWorkRepositoryInterface
{
    ...
    {
        return $this->managerWork->getListWork($filters);
    }
    ...
    public function add(array $data)
    {
        ...
        if (!$idPhase & isset($data['manage_project_id']) && $data['manage_project_id'] != null && $data['manage_project_id'] != '') {
            ...
            $data['manage_project_phase_id'] = $defaultPhase['manage_project_phase_id'];
        }
    ...
    */
    ...
    public function changeTabDetailWork($data)
    {
        try {
            ...
            $mWorkLocation = app()->get(ManageWorkLocationTable::class);

            //Lấy vị trí công việc
            $dataLocation = $mWorkLocation->getLocationByWork($data['id']);

            switch ($data['view']) {
                case 'comment':
                    $listComment = $this->getListComment($data['id']);
                    $view = view('manager-work::managerWork.detail_comment', [
                        'detail' => $detail,
                    ...
                    break;
                case 'sub_task':
                    if (isset($data['manage_project']) && $data['manage_project'] != 'undefined') {
                        if ($detail['parent_id'] == null) {
                            ...
                            $project = $rProject->getDetailFix($data['manage_project']);
                            $listStaffManage = $mManageProjectStaff->getListAdmin($data['manage_project'], 'administration');
                        ...
                        if ($detail['parent_id'] == null) {
                            ...
                            $view = view('manager-work::managerWork.detail-child-work', [
                                'detail' => $detail,
                                'listWorkChild' => $listWorkChild,
                                'listStatus' => $listStatus
                            ])->render();
                ...
                case 'document':
                    ...
                    $view = view('manager-work::managerWork.detail-document', [
                        'detail' => $detail,
                ...
                case 'remind':
                    ...
                    $view = view('manager-work::managerWork.detail-remind', [
                        'detail' => $detail,
                ...
                case 'history':
                    ...
                    $view = view('manager-work::managerWork.detail-history', [
                        'detail' => $detail,
                        'listStaff' => $listStaff
                    ])->render();
                    ...
                    break;
                case 'location':
                    $view = view('manager-work::managerWork.detail-location', [
                        'detail' => $detail,
    ...
    public function showFormComment($data)
    {
        try {
            ...
            $view = $view = view('manager-work::managerWork.append.append-form-chat', ['manage_comment_id' => $data['manage_comment_id']])->render();
    ...
    public function searchListHistory($data)
    {
        try {
            ...
            $view = $view = view('manager-work::managerWork.append.list-history', ['listHistory' => $listHistory])->render();
    ...
    public function searchDocument($data)
    {
        try {
            ...
            $view = $view = view('manager-work::managerWork.append.append-list-document', ['listDocument' => $listDocument])->render();
            return [
                'error' => false,
                'view' => $view
            ];
    ...
    public function showPopupUploadFile($data)
    {
        try {
            ...
            if (isset($data['manage_document_file_id'])) {
                $detailFile = $mManageDocumentFile->getDetail($data['manage_document_file_id']);
            } else {
                $detailFile = null;
            }

            $viewPopup = 'manager-work::managerWork.popup.popup-upload-file';
        ...
        try {
            ...
            if (isset($data['manage_document_file_id'])) {
                foreach ($data['document'] as $itemDocument) {
                    ...
                    $mManageDocumentFile->updatedFileDocument($dataImage, $data['manage_document_file_id']);
                }
            } else {
                foreach ($data['document'] as $itemDocument) {
                    $dataImage = [
            ...
            $detailFile = null;
            $view = $view = view('manager-work::managerWork.popup.popup-upload-file', ['detailFile' => $detailFile])->render();
    ...
    public function showPopupRemindPopup($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.popup.remind-work', ['data' => $data, 'detail' => $detail, 'workDetail' => $workDetail])->render();
    ...
    public function addRemindWork($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.popup.remind-work', ['data' => $data, 'detail' => $detail])->render();
    ...
    public function searchRemind($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.append.append-list-remind', ['listRemind' => $listRemind])->render();
    ...
    public function showPopupWorkChild($data)
    {
        try {
            ...
            $listWork = $mManageWork->getAllParentPagination(['parent_id' => isset($data['parent_id']) ? $data['parent_id'] : '']);
            ...
            if (isset($data['view']) && in_array($data['view'], ['shift', 'my_shift'])) {
                $dataShift = [
            ...
            $arrStaff = [];
            if ($detail == null) {
                if ($parentWork != null && $parentWork['manage_project_id'] != null) {
                    $dataStaff = $this->getListStaffByProject($parentWork['manage_project_id']);
...
//                            $getStaff = $mStaff->getListStaffByBranchDepartment($v['branch_id'], $v1['department_id'])->toArray();
            ...
            $view = view('manager-work::managerWork.popup.popup-work', [
...
//                if (isset($data['time_start']) && strlen(explode(':',$data['time_start'])[0]) == 1) {
//                    $data['time_start'] = '0'.$data['time_start'];
//                }
//                if (isset($data['time_end']) && strlen(explode(':',$data['time_end'])[0]) == 1) {
//                    $data['time_end'] = '0'.$data['time_end'];
//                }

//                if (isset($data['type_copy'])) {
...
//                    $dataWork['manage_work_code'] = $this->codeWork();
                    ...
                    $idWork = $data['manage_work_child_id'];
//                    $detailOld = $mManageWork->getDetail($idWork);
                    $detailOld = $this->getDetail($idWork);
...
//                        if (isset($dataWork['processor_id']) && (isset($detailOld) && $detailOld['processor_id'] != $dataWork['processor_id']) && !isset($detailOld)) {
                        if (isset($dataWork['processor_id']) && (isset($detailOld) && $detailOld['processor_id'] != $dataWork['processor_id'])) {
                            $dataNoti = [
                                'key' => 'work_assign',
                                'object_id' => $idWork,
                            ];
            ...
            return [
                ...
                'route' => route('manager-work.detail',['id' => $idWork]),
                'data' => [
                    'manage_work_id' => $idWork,
...
//        $codeWork = $detailProject['prefix_code'].'_' . Carbon::now()->format('Ymd') . '_';
    ...
    public function searchWork($data)
    {
        try {
            ...
            $template = 'manager-work::managerWork.append.append-list-work-child';
            $project = null;
            $listStaffManage = [];
            if (isset($data['manage_project']) && $data['manage_project'] != 'undefined') {
                $rProject = app()->get(ProjectRepositoryInterface::class);
                ...
                $template = 'manager-project::work.append.append-list-work-child';
            }
    ...
    public function export($data)
    {
        ...
        if (count($data['list']) > 0) {
            ...
            foreach ($data['list'] as $item) {
                ...
                if(isset($item->create_object_type)){
...
//                    'manage_type_work_icon' => $item['manage_type_work_icon'] != '' ? ($item['manage_type_work_icon']) : asset('static/backend/images/service-card/default/hinhanh-default3.png'),
    ...
    public function changeCustomer($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.append.option-select-customer', ['arrCustomer' => $arrCustomer, 'typeCustomer' => $data['typeCustomer'], 'detail' => $detail])->render();
    ...
    public function popupChangeFolder($data)
    {
        try {
            ...
            $view = view('manager-work::managerWork.popup.popup-change-folder', ['detail' => $detail, 'access_token' => $access_token, 'brandCode' => $brandCode, 'detailConfig' => $detailConfig])->render();
...
//                    'file_path' => env('FOLDER').'/'.$data['old_folder_display_name'],
    ...
    {
        try {
            $view = 'manager-work::managerWork.popup.popup-list-staff';
    ...
    public function searchPagePopupStaff($data)
    {
        try {
            ...
            $view = 'manager-work::managerWork.append.get-list-staff';
    ...
    public function changeBranchStaff($data)
    {
        ...
        $view = view('manager-work::managerWork.append.option-staff', [
            'listStaff' => $listStaff
        ])->render();
    ...
    public function checkDateWorkProject($data)
    {
        ...
        $detailProject = $mProject->getDetailProject($data['manage_project_id']);

        if (!isset($detailProject['date_start']) || !isset($data['date_start'])) {
            return [
                'error' => false
            ];
        }
        if ($detailProject['date_start'] == null || $detailProject['date_start'] == null) {
            return [
                'error' => false
            ];
        }
        $detailProject['date_start'] = Carbon::createFromFormat('Y-m-d', $detailProject['date_start'])->format('Y-m-d 00:00:00');
        $detailProject['date_end'] = Carbon::createFromFormat('Y-m-d', $detailProject['date_end'])->format('Y-m-d 23:59:59');
        $data['date_start'] = Carbon::createFromFormat('d/m/Y H:i', $data['date_start'])->format('Y-m-d H:i:00');
        $data['date_end'] = Carbon::createFromFormat('d/m/Y H:i', $data['date_end'])->format('Y-m-d H:i:00');

        if ($detailProject['date_start'] > $data['date_start'] || $detailProject['date_end'] < $data['date_end']) {
            return [
                'error' => true,
                'title' => __('Hạn công việc có thời gian lớn hơn hoặc nhỏ hơn thời gian thực hiện của dự án. Bạn xác nhận muốn tiếp tục')
            ];
        } else {
            return [
                'error' => false
    ...
    {
        $mManageWork = app()->get(ManagerWorkTable::class);
        return $mManageWork->getAllParentPagination($data);
    }
    ...
    public function changeParentTask($data)
    {
        try {
            $mManageWork = app()->get(ManagerWorkTable::class);
            $detail = $mManageWork->getDetail($data['parent_id']);

            return [
                'error' => false,
                'manage_type_work_id' => isset($detail) ? $detail['manage_type_work_id'] : '',
                'manage_project_id' => isset($detail) ? $detail['manage_project_id'] : ''
            ];
        } catch (Exception $e) {
            return [
                'error' => true,
    ...
    public function changeListStaff($data)
    {
        ...
        $listStaff = $this->getListStaffByProject($data['manage_project_id']);
...
//                            $getStaff = $mStaff->getListStaffByBranchDepartment($v['branch_id'], $v1['department_id'])->toArray();
        ...
        $viewGroup = view('manager-work::managerWork.append.option-staff-group', [
            'dataOptionSupport' => $dataOptionSupport,
        ])->render();

        $viewPhase = view('manager-work::managerWork.append.option-phase', [
            'listPhase' => $listPhase,
        ])->render();
    ...
    public function showPopStaffSupport($data)
    {
        ...
        $html = \View::make('manager-work::managerWork.popup.pop-staff-support', $list)->render();
...
//            ],
    ...
    public function removeStaffSupport($input)
    {
        ...
        //Put session chính
        session()->put('staff_support', $arrCheck);
    }
}
