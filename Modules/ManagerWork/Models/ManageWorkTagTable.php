...
class ManageWorkTagTable extends Model
{
    ...
    protected $fillable = ['manage_work_tag_id', 'manage_work_id', 'manage_tag_id','created_by',
        'updated_by', 'created_at', 'updated_at'];

    protected function _getList($filters = [])
    {
        $query = $this->select('manage_work_tag_id', 'manage_work_id', 'manage_tag_id','created_by',
        'updated_by', 'created_at', 'updated_at')
    ...
    public function getName(){
        $oSelect= self::select("manage_work_tag_id","manage_work_id")->get();
        return ($oSelect->pluck("manage_work_id","manage_work_tag_id")->toArray());
    }

    public function add(array $data)
    {
        $oCustomerGroup = $this->create($data);
        return $oCustomerGroup->manage_work_tag_id;
    }
