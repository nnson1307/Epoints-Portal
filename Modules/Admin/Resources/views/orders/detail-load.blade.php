            ...
            <div class="m-portlet__head-tools">
                <div class="m-form__actions m--align-right">
                    ...
                    @if(in_array($order['process_status'], ['new']) && $order['receive_at_counter'] != 1)
                        <button class="btn btn-info color_button m--margin-left-10" type="button"
                    ...
                    @if($order['process_status'] == "new")
                        <a class="btn btn-info color_button m--margin-left-10"
                           href="{{route('admin.order.receipt-after', $order['order_id'] . '?type=order')}}">
                            ...
                            <div class="table-responsive">
                                <table class="table m-table">
                                    ...
                                    <tr class="bg">
                                        ...
                                        <th class="tr_thead_od_detail"
                                            style="text-align: right">{{__('TỔNG TIỀN')}}</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($oder_detail as $item)
                                        <tr>
                                            <td style="text-align: center" style="vertical-align: middle;">
                                                <img class="m--bg-metal m-image img-sd myImg"
                                            ...
                                            <td style="vertical-align: middle;">
                                                @if($item['object_type'] == "service")
                                                    <a target="_blank"
                                                       href="{{route("admin.service.detail",$item['object_id'])}}">
                                                        {{$item['object_name']}}
                                                    </a>
                                                @elseif($item['object_type'] == "product")
                                                    <a target="_blank"
                                                       href="{{route('admin.product-child-new.detail',$item['object_id'])}}">
                                                        {{$item['object_name']}}
                                                    </a>
                                                @elseif($item['object_type'] == "service_card")
                                                    <a target="_blank"
                                                       href='{{route("admin.service-card.detail",$item['object_id'])}}'>
                                                        {{$item['object_name']}}
                                                    </a>
                                            ...
                                            <td class="text-center"
                                                style="vertical-align: middle;">{{number_format($item['quantity'], 1)}}</td>
                                            <td class="text-center"
                                                style="vertical-align: middle;">{{number_format($item['discount'], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}} @lang('đ')</td>
                                            ...
                                            @if(isset($receiptOrder) && count($receiptOrder) > 0)
                                                @foreach ($receiptOrder as $key => $v)
                                                    <tr>
                                                        <td>
                                                            <a target="_blank"
                                                               href="{{route('receipt.show', $v['receipt_id'])}}">
                                                                {{$v['receipt_code']}}
                                                            </a>
                                                        </td>
                                                        ...
                                                        <td>{{$v['full_name']}}</td>
                                                        <td>{{number_format($v['amount_paid'], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}</td>
