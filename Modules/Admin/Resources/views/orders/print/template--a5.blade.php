            ...
            <div id="PrintArea">
                <div class="widhtss">
                    ...
                    <div class="mm-mauto tientong roww">
                        ...
                        <div class="coll-3">
                            <span style="margin-left: 20px"
                                  class="text-center font-size-13">{{__('Ngày')}} {{date('d')}}
                    ...
                    <table style="width:100%">
                            ...
                            <td colspan="5">
                                <div class="row_custom">
                                    ...
                                    <div class="coll-4 text-right" style="font-size: 15px;">
                                        {{number_format($order['discount_member']+$km, isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}} {{__('VNĐ')}}
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="5">
                                <div class="row_custom">
                                    ...
                                    <div class="coll-4 text-right" style="font-size: 15px;">
                                        {{number_format($order['discount']+$km, isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}} {{__('VNĐ')}}
                                    </div>
                                </div>
                            </td>
                        </tr>
                            ...
                            <td colspan="5">
                                <div class="row_custom">
                                    ...
                                    <div class="coll-4 text-right" style="font-size: 15px; font-weight: bold;">
                                        {{number_format($order['amount']+$km, isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}} {{__('VNĐ')}}
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="5">
                                <div class="row_custom">
                                    <div class="coll-8" style="font-size: 15px;">
                                        {{__('TỔNG TIỀN KHÁCH TRẢ')}}: <br>
                                        @if (isset($receipt_detail) && $receipt_detail != null)
                                            @foreach($receipt_detail as $paymentMethod)
                                                <span style="margin-left: 15px;">{{$paymentMethod['payment_method_name']}}</span> <br>
                                            @endforeach
                                        @endif
                                    </div>
                                    <div class="coll-4 text-right" style="font-size: 15px;">
                                        {{number_format($totalCustomerPaid, isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}} {{__('VNĐ')}} <br>
                                        @if (isset($receipt_detail) && $receipt_detail != null)
                                            @foreach($receipt_detail as $paymentMethod)
                                                {{number_format($paymentMethod['amount'], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}} {{__('VNĐ')}} <br>
                                            @endforeach
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="mm-mauto tientong font-size-15 roww" style="margin-top: 2px !important;">
                        {{__('TỔNG TIỀN KHÁCH TRẢ')}} {{__('VIẾT BẰNG CHỮ')}}:
                        <span class="first-text-upper">{{$text_total_amount_paid}}
                            {{__('đồng')}}.
                        </span>
                    </div>
                    ...
                    <div class="mm-mauto tientong font-size-15 roww">
                        <strong class="coll-5 font-size-13"
                                style="font-weight: 300;font-size: 13px;margin-left: 60px">{{__('(Ký, ghi rõ họ tên)')}}</strong>
                        <strong class="coll-5 font-size-13"
                                style="font-weight: 300;font-size: 13px;margin-right: 25px">{{__('(Ký, đóng dấu, ghi rõ họ tên)')}}</strong>
                    </div>
                    <br>
                    <br>
                    <br>
