...
<div class="m-datatable m-datatable--default">
    <div class="m-datatable__pager m-datatable--paging-loaded">
        <ul class="m-datatable__pager-nav">
            ...
            @endif
            @for ($i=1;$i<(int)(count($LIST)/10)+2;$i++)
                @if($i==1)
            ...
            @if($page<(int)(count($LIST)/10)+1)
                ...
                <li><a title="Last" onclick="firstOrLastPageSearch({{(int)(count($LIST)/10)+1}})"
        ...
        <div class="m-datatable__pager-info">
            <div class="btn-group bootstrap-select m-datatable__pager-size" style="width: 70px;">
                ...
                @endphp
                {!! Form::select('display', $display,10, ['class' => 'selectpicker m-datatable__pager-size', 'data-width' => '70px']) !!}
            </div>
            <span class="m-datatable__pager-detail">
                {{__('Hiển thị')}} {{count($LIST)/10}} - {{($page-1)*10 + count($LIST)}} {{__('của')}} {{ count($data) }}
            </span>
        </div>
    </div>
</div>
