...
<div class="book">
    <div class="page">
        <div class="subpage">
            ...
            <div class="form-group text-center">
                <span style="font-size: 23px; font-weight: bold;">@lang('HOÁ ĐƠN THANH TOÁN CÔNG NỢ')</span>
            </div>
            <div class="form-group row_custom clearfix">
                <div class="col-print-7">
                    <div>
                            <span class="ss-font-size-10 roww"><strong>{{__('Mã hồ sơ')}}</strong>: {{$debt['profile_code']}}
                            </span>
                    </div>
                    <div>
                        <span class="ss-font-size-10 roww"><strong>{{__('Mã khách hàng')}}</strong>: {{$debt['customer_code']}}
                            </span>
                    </div>
                    <span class="font-size-14">
                        <strong>@lang('Khách hàng'):</strong> @if($debt['customer_id']!=1)
                            {{$debt['customer_name']}}
                        @else
                    ...
                    <span class="font-size-14">
                        <strong>@lang('Điện thoại'):</strong> {{$debt['customer_phone']}}
                    </span> <br>
{{--                    <span class="font-size-14">--}}
{{--                        <strong>@lang('Địa chỉ'):{{$debt['customer_phone']}}</strong>--}}
{{--                    </span>--}}
                </div>
                <div class="col-print-5">
                    <span class="font-size-14">
                        <strong>@lang('Nhân viên'):</strong> {{$receipt['full_name']}}
                    </span> <br>
                    <span class="font-size-14">
                        <strong>@lang('Ngày giờ bán'):</strong> {{\Carbon\Carbon::parse($receipt['created_at'])->format('d/m/Y H:i:s')}}
                    </span> <br>
                    <span class="font-size-14">
                        <strong>@lang('Số hoá đơn'):</strong> {{$debt['debt_code']}}
                    </span>
                </div>
            </div>
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="row_custom">
                                    ...
                                    <div class="col-print-4 text-right" style="font-size: 14px;">
                                        {{number_format($amount_bill, isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    @if ($configPrintBill['is_total_discount'] == 1)
                        <tr>
                            <td colspan="7">
                                <div class="row_custom">
                                    <div class="col-print-8" style="font-size: 14px;">
                                        @lang('Tổng tiền đã giảm') (@lang('VNĐ'))
                                    </div>
                                    <div class="col-print-4 text-right" style="font-size: 14px;">
                                        -0
                                    </div>
            ...
            <div class="form-group" style="font-style: italic">
                @lang('Ghi chú'): <span style="font-weight: bold;">{{$receipt['note']}}</span>
            </div>
            <div class="form-group">
