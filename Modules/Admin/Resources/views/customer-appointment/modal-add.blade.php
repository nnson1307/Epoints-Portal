<div class="modal" id="modal-add" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg-appointment" role="document" style="width: 1200px;">
            ...
            <div class="modal-body">
                <div class="m-widget4  m-section__content" id="m_blockui_1_content">
                    <form action="" method="post" id="form-add" novalidate="novalidate" autocomplete="off">
                        {!! csrf_field() !!}
                        <input type="hidden" id="customer_hidden" name="customer_hidden" value="{{$customerLoad != null ? $customerLoad['customer_id'] : ''}}">
                                        ...
                                        <div class="input-group">
                                            <div class="autocomplete" style="width:100%;">
                                                <input type="text" class="form-control"
                                                       onkeydown="customer_appointment.chooseCustomer(this)" {{$customerLoad != null  ? 'disabled': ''}}
                                                       name="phone1" id="phone1" placeholder="Vd: 0791234567" value="{{$customerLoad != null ? $customerLoad['phone1'] : ""}}">
                                            </div>
                                        </div>
                                    ...
                                    <div class="form-group m-form__group col-lg-4">
                                        ...
                                        <div class="input-group">
                                            <div class="m-input-icon m-input-icon--right">
                                                <input class="form-control" name="full_name" id="full_name" {{$customerLoad != null  ? 'disabled': ''}}
                                                       placeholder="Vd: Nguyễn Văn A" value="{{$customerLoad != null ? $customerLoad['full_name'] : ""}}">
...
{{--                                                                class="la la-user"></i></span></span>--}}
                                    ...
                                    <div class="form-group m-form__group col-lg-4">
                                        ...
                                        <div class="input-group">
                                            <select id="customer_group_id" name="customer_group_id" class="form-control" style="width:100%;" {{$customerLoad != null  ? 'disabled': ''}}>
                                                <option></option>
                                                @foreach($optionGroup as $key=>$value)
                                                    <option value="{{$value['customer_group_id']}}"
                                                            {{$customerLoad != null && $customerLoad['customer_group_id'] == $value['customer_group_id'] ? 'selected': ''}}>{{$value['group_name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                            ...
                                            <select class="form-control" id="branch_id_modal" name="branch_id"
                                                ...
                                                @foreach($optionBranch as $v)
                                                    <option value="{{$v['branch_id']}}"
                                                            {{$v['branch_id'] == Auth()->user()->branch_id ? 'selected': ''}}>{{$v['branch_name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    @endif
                                    ...
                                    <table class="table m-table m-table--head-separator-metal" id="table_quantity">
                                        ...
                                        @if (isset($listMemberCard) && count($listMemberCard) > 0)
                                            <tr class="tr_quantity tr_card">
                                                ...
                                                <td>
                                                    <select class="form-control service_id" name="service_id" id="service_id_2"
                                                            style="width: 100%"
                                                            multiple="multiple">
                                                        <option></option>
                                                        @foreach($listMemberCard as $k => $v)
                                                            <option value="{{$v['customer_service_card_id']}}">{{$v['card_name']}}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td style="{{session()->get('brand_code') != 'giakhang' ? '': 'display:none;'}}">
                            ...
                            <div class="m-portlet__foot m-portlet__no-border m-portlet__foot--fit">
                                <div class="m-form__actions m-form__actions--solid m--align-right w-100">
                                    <a href="javascript:void(0)" data-dismiss="modal"
                                       onclick="customer_appointment.out_modal()"
                                    ...
                                    @if($customerLoad == null)
                                        <button type="button"
                                                class="btn btn-addNewContinued btn-info m-btn m-btn--icon m-btn--wide m-btn--md btn_new color_button son-mb m--margin-left-10"
                                                onclick="customer_appointment.addNewContinued()">
                                                <span>
