...
@section('content')
                                        ...
                                        @if(count($data_detail)>0)
                                            @foreach($data_detail as $key => $v)
                                                <tr class="tr_table">
                                                    ...
                                                    <td class="td_vtc"
                                                        style="color: #000; font-weight: 500;width: 100px;">
                                                        {{$v['service_name']}}
                                                        ...
                                                        <input type="hidden" name="object_code" value="{{$v['service_code']}}">

                                                    </td>
                                                    <td class="td_vtc" style="width: 130px;">
                                                        <input class="form-control price" name="price" value="{{number_format($v['price'], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}" id="price_{{$key + 1}}">
                                                    </td>
                                                    <td class="td_vtc" style="width: 90px;">
                                                        ...
                                                        @if($v['object_type'] == 'member_card')
                                                            <input type="hidden" name="quantity_hid"
                                                                   value="{{$v['number_using'] != 0 ? $v['number_using'] - $v['count_using'] : 0}}">
                                                        @else
                                                            <input type="hidden" name="quantity_hid" value="0">
                                                        @endif
                                                    </td>
                                                    <td class="discount-tr-{{$v['object_type']}}-{{$key+1}} td_vtc"
                                                        style="text-align: center; width: 130px;">
                                                        <input type="hidden" name="discount" class="form-control discount" value="0" maxlength="11">
                                                        <input type="hidden" name="voucher_code" value="">

                                                        @if($is_edit_full == 1)
                                                            @if ($customPrice == 0 && $v['object_type'] != 'member_card')
                                                                <a class="btn ss--button-cms-piospa m-btn m-btn--icon m-btn--icon-only"
                                                                   href="javascript:void(0)"
                                                                   onclick="order.modal_discount('{{$v['price']*$v['quantity']}}','{{$v['service_id']}}', 1, '{{$key+1}}')"><i
                                                    ...
                                                    <td class="td_vtc" style="width: 50px;">
                                                        ...
                                                        <a class="remove" href="javascript:void(0)"
                                                           {{$is_edit_full == 0 ? 'hidden' : ''}}
                                                           style="color: #a1a1a1"><i class="la la-trash"></i></a>
                                                    </td>

                                                    <input type="hidden" id="numberRow">
                                                    <input type="hidden" name="note" id="note_{{$key+1}}">
                                                </tr>
                                                <tr class="tr_note_child_{{$key+1}}">
                                                    <td></td>
                                                    <td colspan="2">
                                                        <span id="note_text_{{$key+1}}">@lang('Ghi chú/Dịch vụ thêm')...</span>
                                                        <a href="javascript:void(0)"
                                                           onclick="order.showPopupAttach('{{$key+1}}', '{{$v['object_type']}}', '{{$v['service_id']}}', '{{$v['service_name']}}', '{{$v['price']}}')"
                                                    ...
                                                    <td colspan="2" class="td_staff">
                                                        <select class="form-control staff"
                                                                ...
                                                                name="staff_id" multiple="multiple" style="width:80%;">
                                                            <option></option>
                    ...
                    <div class="row">
                        @if($is_payment_order == 1)
                            <div class="form-group col-lg-4">
                                <button type="button" onclick="processPayment.createPaymentAction('appointment')"
                                        id="btn_order"
                        ...
                        @if($is_update_order == 1)
                            <div class="form-group col-lg-4">
                                <button type="submit" id="btn_add" onclick="order.save_order()"
    ...
    <input type="hidden" id="order_source_id" value="1">
    <input type="hidden" id="custom_price" name="custom_price" value="{{$customPrice}}">
    <input type="hidden" id="receive_at_counter">
    <input type="hidden" id="type_time_hidden" name="type_time_hidden">
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/admin/customer-appointment/receipt.js?v='.time())}}" type="text/javascript"></script>
