        ...
        <ul class="m-datatable__pager-nav" style="float: right">
            @if(count($dataOrder)>10)
                @if($pageOrder>1)
                    ...
                    <li><a onclick="Paginate.pageClickOrder({{$pageOrder-1}})" title="Previous"
                ...
                @for ($i=1;$i<$totalPage;$i++)
                    ...
                    @endif
                @endfor
                @if($pageOrder<$totalPage-1)
                    <li><a title="Next" class="m-datatable__pager-link" onclick="Paginate.pageClickOrder({{$pageOrder+1}})"
        ...
        <div class="m-datatable__pager-info" style="float: left">
            <span class="m-datatable__pager-detail">
                @if(count($dataOrder)>0)
                    {{__('Hiển thị')}} {{($pageOrder-1)*10+1}}
                    - {{($pageOrder-1)*10 + count($listOrder)}}
