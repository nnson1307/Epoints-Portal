    ...
    <table class="table table-striped m-table ss--header-table">
        ...
        @if(isset($LIST))
            @foreach ($LIST as $key => $value)
                <tr>
                    @if(isset($page))
                        <td class="ss--font-size-13 ss--text-center">{{ ($page-1)*10 + $key+1}}</td>
                    @else
                        <td class="ss--font-size-13 ss--text-center">{{$key+1}}</td>
                    @endif
                    <td class="ss--font-size-13">
                        <a href="{{route('admin.campaign.detail',$value['campaign_id'])}}" class="m-link"
                           style="color:#464646">
                            {{$value['sms_campaign_name']}}
                        </a>
                    </td>
                    <td class="ss--font-size-13 ss--text-center ss--nowrap">{{$value['created_by']}}</td>
                    <td class="ss--font-size-13 ss--text-center ss--nowrap">{{(new DateTime($value['created_at']))->format('d/m/Y')}}</td>
                    <td class="ss--font-size-13 ss--text-center ss--nowrap">{{$value['sent_by']}}</td>
                    <td class="ss--font-size-13 ss--text-center ss--nowrap">{{($value['time_sent'] != null)?(new DateTime($value['time_sent']))->format('d/m/Y'):''}}</td>
