        ...
        @if(isset($LIST ))
            @foreach ($LIST as $key => $item)
                <tr>
                    ...
                    <td class="ss--font-size-13" style="text-align: right;width: 100px">
                        {{number_format($item['price'],isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}
                    </td>
                    @if(Auth::user()->is_admin==1)
                        <td class="ss--font-size-13 ss--text-center" style="width:100px;max-width: 100px">
                            {{number_format($item['quantity'],0,"",",")}}
                            {{--<label  style="text-align:center;width:70px;max-width: 100px"></label>--}}
                        </td>
                    @endif
