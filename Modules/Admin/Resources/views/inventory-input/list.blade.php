        ...
        @if (isset($LIST))
            @foreach ($LIST as $key=>$item)
                <tr data-id="{{ ($key+1) }}" class="ss--nowrap">
                    ...
                    @endif
                    <td class="ss--font-size-13"><a href="{{route('admin.inventory-input.detail',$item['id'])}}"
                    ...
                    <td class="ss--font-size-13 ss--max-width-200">{{ $item['warehouseName'] }}</td>
                    ...
                    <td class="pull-right ss--font-size-13 ss--nowrap">
                        @if($item['status']!='success'&&$item['status']!='cancel')
                            @if(Auth::user()->is_admin==1||in_array('admin.inventory-input.edit',session('routeList')))
                                <a href="{{route('admin.inventory-input.edit',$item['id'])}}"
                            ...
                            @endif
                            @if(in_array('admin.inventory-input.remove',session('routeList')))
                                <button onclick="InventoryInput.remove(this, '{{ $item['id'] }}')"
