            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Mã phiếu')}}: {{$inventoryOutput->code}}
                        </label>
                    </div>
                    @if($inventoryOutput->order_code != null)
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Mã đơn hàng')}}: <a target="_blank" href="{{route('admin.order.detail',$inventoryOutput['object_id'])}}"> {{$inventoryOutput->order_code}}</a>
                        </label>
                    </div>
                    @endif
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Kho xuất')}} hàng: {{$inventoryOutput->warehouseName}}
                        </label>
                    </div>
                    ...
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Trạng thái')}}:
                            @if($inventoryOutput->status=='new')
                                <span>{{__('Mới')}}</span>
                            @elseif($inventoryOutput->status=='success')
                                <span>{{__('Hoàn thành')}}</span>
                            @elseif($inventoryOutput->status=='draft')
                                <span>{{__('Lưu nháp')}}</span>
                            @elseif($inventoryOutput->status=='cancel')
                                <span>{{__('Hủy')}}</span>
                            @elseif($inventoryOutput->status=='inprogress')
                    ...
                    <div class="form-group m-form__group">
                        <label>
                            Loại: @if($inventoryOutput->type=='normal')
                                <span>{{__('Thường')}}</span>
                            @elseif($inventoryOutput->type=='retail')
                                <span>{{__('Bán lẻ')}}</span>
                            @elseif($inventoryOutput->type=='transfer')
                                <span>{{__('Chuyển kho')}}</span>
                            @elseif($inventoryOutput->type=='checking')
                                <span>{{__('Kiểm kho')}}</span>
                            @elseif($inventoryOutput->type=='return')
                                <span>{{__('Hủy')}}</span>
                            @endif
                ...
                <div class="col-lg-6">
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Người tạo')}}: {{$inventoryOutput->user}}
                        </label>
                    </div>
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Ngày tạo')}}: {{(new DateTime($inventoryOutput->createdAt))->format('d/m/Y')}}
                        </label>
                    </div>
                  
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Tổng số lượng')}}: {{$inventoryOutput->total_quantity}}
                        </label>
                    </div>
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Tổng tiền')}}: {{number_format($inventoryOutput->total_money)}}
                        </label>
                    </div>
                    <div class="form-group m-form__group">
                        <label>
                            {{__('Ghi chú')}}: {{$inventoryOutput->note}}
                        </label>
                    </div>
                </div>
                <div class="col-lg-12">
                            ...
                            <table id="add-product-version"
                                ...
                                @if(isset($LIST))
                                    @foreach($LIST as $key=>$value)
                                        ...
                                        @if(isset($listSerial[$value['inventory_output_detail_id']]) && $value['inventory_management'] == 'serial' && count($listSerial[$value['inventory_output_detail_id']]) != 0)
                                            <tr class="ss--font-size-13 ss--nowrap">
                                                <td></td>
                                                <td colspan="5">
                                                ...
                                                <td class="text-center">
                                                    @if(count($listSerial[$value['inventory_output_detail_id']]) > 10)
