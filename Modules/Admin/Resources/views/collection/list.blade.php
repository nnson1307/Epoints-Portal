    ...
    <div class="m-portlet">
        <div class="m-portlet__head hu-first-uppercase text-uppercase">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    ...
                    <h3 class="m-portlet__head-text">
                        {{__('danh sách collection check in')}}
                    </h3>
                </div>
            </div>
            <div class="m-portlet__head-tools">
                ...
                @if(1||in_array('admin.collection.ajax-add-modal',session('routeList')))
                    <a href="javascript:void(0)"
                       class="btn ss--button-cms-piospa m-btn m-btn--icon m-btn--pill btn_add_pc btn-sm m--margin-right-5">
                        <span class="ajax-collection-add-modal ajax submit" method="POST"
                              action="{{route('admin.collection.ajax-add-modal')}}">
						    <i class="fa fa-plus-circle"></i>
							<span class=""> {{__('Thêm collection')}}</span>
                        </span>
                    </a>
        ...
        <div class="m-portlet__body">
            @include('admin::collection.filters')
            <div class="table-content collection-table">
...
@section('after_script')
    ...
    <script src="{{asset('static/backend/js/admin/collection/script.js?v='.time())}}" type="text/javascript"></script>
