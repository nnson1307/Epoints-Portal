...
namespace Modules\Admin\Repositories\CustomerGroupFilter;
...
use Modules\Admin\Models\CustomerGroupConditionTable;
...
class CustomerGroupFilterRepository implements CustomerGroupFilterRepositoryInterface
{
                ...
                if (isset($value['value']) && $value['value'] != null) {
                    $data = [
                        ...
                        'use_service_card' => $value['condition'] == 17
                            ? implode(',', $value['value']) : null,
                        'address' => $value['condition'] == 18 && isset($value['value']) ? json_encode($value['value']) : null,
                        'type_customer' => $value['condition'] == 19 ? implode(',', $value['value']) : null,
                        'group_customer' => $value['condition'] == 20 ? implode(',', $value['value']) : null,
                        'source_customer' => $value['condition'] == 21 ? implode(',', $value['value']) : null,
                        'created_at' => date('Y-m-d H:i:s'),
                        'updated_at' => date('Y-m-d H:i:s'),
                    ];
        ...
        foreach ($groupDetail as $item) {
            if ($item['group_type'] == $type) {
                if ($item['condition_id'] == 1) {
                    ...
                    //                    $temp7 = [];
                    //                    if (count($customer7) > 0) {
                    //                        //Danh sách KH sử dụng các dịch vụ.
                    //                        $tempCustomer = [];
                    //                        foreach ($customer7 as $c) {
                    //                            if ($c['process_status'] == 'paysuccess') {
                    //                                if ($c['object_type'] == 'service') {
                    //                                    if (in_array(
                    //                                        $c['object_id'], $arrayService2
                    //                                    )
                    //                                    ) {
                    //                                        $tempCustomer[] = $c['customer_id'];
                    //                                    }
                    //                                }
                    //                            }
                    //                        }
                    //                        //Danh sách KH không sử dụng các dịch vụ.
                    //                        foreach ($customer7 as $c) {
                    //                            if ( ! in_array($c['customer_id'], $temp7)) {
                    //                                if ( ! in_array(
                    //                                    $c['customer_id'], $tempCustomer
                    //                                )
                    //                                ) {
                    //                                    $temp7[] = $c['customer_id'];
                    //                                }
                    ...
                    //                        //Danh sách KH không sử dụng các dịch vụ.
                    //                        foreach ($customer9 as $c) {
                    //                            if ( ! in_array($c['customer_id'], $temp9)) {
                    //                                if ( ! in_array(
                    //                                    $c['customer_id'], $tempCustomer
                    //                                )
                    //                                ) {
                    ...
                    $sourceCustomer = explode(',', $item['group_customer']);
                    $customer20 = $this->customer->getCustomerByGroup($sourceCustomer);
                    $temp20 = [];
                    ...
                    $groupCustomer = explode(',', $item['source_customer']);
                    $customer21 = $this->customer->getCustomerBySource($groupCustomer);
    ...
    {
        $listCustomerSource = $this->customerSource->getOptionCustomerSource();
    ...
    public function getListCustomerGroup()
    {
        $listCustomerGroup = $this->customerGroup->getOption();
    ...
    {
        $listProvinces = $this->province->getOptionProvince();
    ...
    {
        $filters = [];
        $filters['id_province'] = $idProvince;
        $listDistrict = $this->district->getOptionDistrict($filters);
        return $listDistrict;
    }
}
