...
class DistrictTable extends Model
{
    ...
    {
        return  $this->select('districtid', 'postcode','name','lat', 'long','type')->where('provinceid', (int)$id)->get();
    }
     ...
     * @return mixed
     */
    public function getDistrictByName($provinceId, $districtName)
    {
        return $this
            ->where("provinceid", $provinceId)
            ->whereRaw('TRIM(LOWER(`name`)) LIKE ? ',[trim(strtolower($districtName)).'%'])
            ->first();
    }
