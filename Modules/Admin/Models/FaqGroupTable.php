...
class FaqGroupTable extends Model
{
     ...
     * @return mixed
     */
    protected function getListCore(&$filter = [])
    {
        $result = $this->leftJoin("faq_group as fgrparent", function ($join) {
            ...
            ->select(
                ...
                $this->table.".faq_group_title_vi",
                $this->table.".faq_group_title_en",
                $this->table.".faq_group_type",
                $this->table.".faq_group_position",
                $this->table.".is_actived",
                "fgrparent.faq_group_title_vi as faq_group_parent_title_vi",
                "fgrparent.faq_group_title_en as faq_group_parent_title_en"
            )
            ->where($this->table.".is_deleted", 0)
//            ->orderBy("{$this->table}.faq_group_position", "asc");
            ->orderBy("{$this->table}.faq_group_id", "DESC");

        //Filter tên nhóm hỗ trợ
        if (isset($filter["search_faq_group_title_vi"]) != "") {
            $search = $filter["search_faq_group_title_vi"];
            $result->where(function ($query) use ($search) {
                $query->where("{$this->table}.faq_group_title_vi", "like", "%" . $search . "%");
            });
            unset($filter["search_faq_group_title_vi"]);
        }

        if (isset($filter["search_faq_group_title_en"]) != "") {
            $search = $filter["search_faq_group_title_en"];
            $result->where(function ($query) use ($search) {
                $query->where("{$this->table}.faq_group_title_en", "like", "%" . $search . "%");
            });
            unset($filter["search_faq_group_title_en"]);
        }
