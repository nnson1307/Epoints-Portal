...
class FaqTable extends Model
{
    ...
    protected function getListCore(array &$filters = [])
    {
        $result = $this->leftJoin("faq_group as fgr", "fgr.faq_group_id", "=", $this->table.".faq_group")
            ...
            ->select(
                ...
                "fgr.faq_group_title_vi",
                "fgr.faq_group_title_en"
            );
        $result->orderBy($this->table.".faq_id",'DESC');
    ...
    {
        $list = $this->with("faqGroup")->where("is_deleted", 0);
    ...
    public function getPolicyTerms($type = 'privacy_policy',$lang = 'vi')
    {
        $result = $this
            ->where($this->table.".is_deleted", 0)
            ->where($this->table.".faq_type", "=", $type)
            ->select(
                $this->table.".faq_type",
                $this->table.".faq_title_{$lang} as faq_title",
                $this->table.".faq_content_{$lang} as faq_content"
            );
        return $result->first();
    }
}
