...
class ProductChildTable extends Model
{
    ...
    protected $fillable
        = [
            ...
            'is_display', 'product_child_sku','product_attribute_json', 'is_master'
        ];
...
//                'product_childs.slug_en',
    ...
    /*
     * get product child by matrix
     */
    public function getProductChildByMatrix($productId , $matrix)
    {
        $select = $this->leftJoin('products', 'products.product_id', '=', 'product_childs.product_id')
            ->select(
                'product_childs.product_child_id as product_child_id',
...
//                $select = $select->whereRaw("JSON_CONTAINS({$this->table}.product_attribute_json = {$item})");
                $select = $select->whereJsonContains($this->table.'.product_attribute_json',$item);
            }
        }
        return $select->first();
    }
    ...
    public function searchProduct($keyword)
    {
        $select = $this->join('products', 'products.product_id', '=', 'product_childs.product_id')
            ...
            ->where('product_childs.product_code', 'like', '%' . $keyword . '%')
            ->orWhere('product_childs.product_child_name', 'like', '%' . $keyword . '%')
            ->orWhere('product_childs.product_child_sku', 'like', '%' . $keyword . '%')
            ->get();
    ...
    public function removeByArrChildId($productId, $arrChildId){
        return $this->where('product_id', $productId)->whereNotIn('product_child_id',$arrChildId)->update(['is_deleted' => 1]);
    }
    ...
    public function getListPagination($filter = []){
        ...
        if (isset($filter['product_child_name'])){
            $oSelect = $oSelect->where('product_child_name','like','%'.$filter['product_child_name'].'%');
            unset($filter['product_child_name']);
        }

        if (isset($filter['product_id'])){
            $oSelect = $oSelect->where('product_id','<>',$filter['product_id']);
    ...
    public function createProductChild(array $data)
    {
