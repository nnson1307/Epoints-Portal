    ...
    protected function _getList(&$filter = [])
    {
        $oSelect = $this
            ...
            ->select('service_card_id',
                ...
                'price',
                'money',
                'is_actived', 'is_surcharge')->where('service_cards.is_deleted', 0);
//        dd($oSelect->get());
    ...
    public function getAll($filter = [])
    {
        $oSelect = $this
            ...
            ->select('service_card_id',
                ...
                'price',
                'money',
                'is_actived', 'is_surcharge')->where('service_cards.is_deleted', 0)->where("is_actived", 1);
...
//                ->orWhere("code","LIKE","%".$filter["keyword"]."%");
    ...
    {
        $select = $this->select('service_card_id', 'name', 'price', 'image', 'code', 'is_surcharge')
    ...
    public function getListAdd($categoryId, $search, $page)
    {
        $ds = $this
            ...
            ->join("service_card_groups as gr", "gr.service_card_group_id", "=", "{$this->table}.service_card_group_id")
            ->where("{$this->table}.is_deleted", 0)
            ->where("{$this->table}.is_actived", 1)
            ->where("gr.is_deleted", 0);

        //Filter nhóm thẻ dịch vụ
        if ($categoryId != null && $categoryId != 'all') {
            $ds->where("{$this->table}.service_card_group_id", $categoryId);
        }
    ...
    {
        return $this->select('service_card_id', 'name', 'price', 'service_card_type', 'image', 'is_actived', 'is_surcharge')
            ->where('is_deleted', 0)->where('is_actived', 1)->orderBy('service_card_id', 'DESC')
            ->get()->toArray();
    }
    ...
    {
        $select = $this
            ->leftJoin('order_details', 'order_details.object_id', '=', 'service_cards.service_card_id')
            ...
            ->where('service_cards.is_deleted', 0)
            ->orderBy("order_details.order_detail_id", "desc");
        return $select->get();
    }
