...
class OrderTable extends Model
{
    ...
    public function _getList(&$filter = [])
    {
        ...
        if (isset($filter['order_source_id'])) {
            $ds = $ds->where('orders.order_source_id', $filter['order_source_id']);
    ...
    public function getOrderAll(&$filter = [])
    {
        $ds = $this
            ->select(
                'orders.order_id as order_id',
        ...
        if (isset($filter['orders$process_status'])) {
            $ds = $ds->where('orders.process_status', $filter['orders$process_status']);
            unset($filter['orders$process_status']);
        }
        // filter list
//        foreach ($filter as $key => $val) {
//            if (trim($val) == '') {
