...
class CustomerTable extends Model
{
    ...
    public function getItem($id)
    {
        $get = $this
            ...
            ->leftJoin('customers as c', 'c.customer_id', '=', "{$this->table}.customer_refer_id")
            ...
            ->select(
                ...
                "c.full_name as refer_name",
                "b.branch_name",
                "{$this->table}.created_at",
                "{$this->table}.updated_at",
                "{$this->table}.date_last_visit"
            )
    ...
    public function getAllCustomer($filter = [])
    {
        $ds = $this
            ...
            ->leftJoin("customer_branch as cb", "cb.customer_id", "=", "{$this->table}.customer_id")
            ->leftJoin("customers as refer", "refer.customer_id", "=", "{$this->table}.customer_refer_id")
            ->where("{$this->table}.customer_id", '!=', 1)
            ->where("{$this->table}.is_deleted", 0)
            ->groupBy("{$this->table}.customer_id");
