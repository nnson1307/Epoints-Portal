<div class="table-responsive">
    <table class="table table-striped m-table m-table--head-bg-default">
        ...
        <tr>
            ...
            <th class="tr_thead_list">{{__('NGÀY BẮT ĐẦU BẢO HÀNH')}}</th>
            <th class="tr_thead_list">{{__('NGÀY KẾT THÚC BẢO HÀNH')}}</th>
        ...
        @if(isset($LIST))
            @foreach ($LIST as $key => $item)
                <tr>
                    ...
                    <td>{{$item['warranty_start_date'] != '' ? \Carbon\Carbon::parse($item['warranty_start_date'])->format('d/m/Y') : ''}}</td>
                    <td>{{$item['warranty_end_date'] != '' ? \Carbon\Carbon::parse($item['warranty_end_date'])->format('d/m/Y') : ''}}</td>
                    ...
                    <td>{{number_format($item['total_not_receipt'], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}</td>
                </tr>
            @endforeach
        @endif
        </tbody>
