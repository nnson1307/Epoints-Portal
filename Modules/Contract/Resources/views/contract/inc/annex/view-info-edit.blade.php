            ...
            @foreach($tabGeneral as $v)
                <div class="form-group m-form__group col-lg-4">
                    ...
                    <div class="input-group">
                        @switch($v['type'])
                            @case('text')
                            @if ($v['key'] == 'contract_name' || $v['key'] == 'contract_no')
                            ...
                            <input type="text" class="form-control m-input date_picker" readonly="" id="{{$v['key']}}"
                                   ...
                                   value="{{\Carbon\Carbon::parse($infoGeneral[$v['key']])->format('d/m/Y')}}">
                    ...
                    <div class="input-group">
                        @switch($v['type'])
                            ...
                            <textarea class="form-control m-input" id="{{$v['key']}}"
                                      ...
                                      cols="5" isValidate="{{$v['is_validate']}}" keyName="{{$v['key_name']}}"
                                      keyType="{{$v['type']}}">{{$infoPayment[$v['key']]}}</textarea>
                            @break
                            ...
                            <input type="number" class="form-control m-input input_int" id="{{$v['key']}}"
                                   name="{{'tab_payment_'. $v['key']}}"
                                   isValidate="{{$v['is_validate']}}" keyName="{{$v['key_name']}}"
                                   keyType="{{$v['type']}}" value="{{$infoPayment[$v['key']]}}">
                            @break
                            ...
                            <input type="text" class="form-control m-input input_float" id="{{$v['key']}}"
                                   ...
                                   isValidate="{{$v['is_validate']}}" keyName="{{$v['key_name']}}"
                                   keyType="{{$v['type']}}"
                                   {{$infoGeneral['is_value_goods'] == 1 && in_array($v['key'], ['total_amount', 'tax', 'discount', 'last_total_amount']) ? 'disabled': ''}}
                                   {{$infoCategory['type'] == 'sell' && in_array($v['key'], ['total_amount', 'tax', 'discount', 'last_total_amount']) ? 'disabled': ''}}
                                   value="{{number_format($infoPayment[$v['key']], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}">
                            @break
                            ...
                            <input type="text" class="form-control m-input date_picker" readonly="" id="{{$v['key']}}"
                                   ...
                                   keyName="{{$v['key_name']}}" keyType="{{$v['type']}}"
                                   value="{{\Carbon\Carbon::parse($infoPayment[$v['key']])->format('d/m/Y')}}">
                            <div class="input-group-append">
                                <span class="input-group-text"><i
                                            class="la la-calendar-check-o glyphicon-th"></i></span>
                            </div>
                            ...
                            @if ($v['key'] == 'payment_method_id')
                                <select class="form-control select" id="{{$v['key']}}"
                                    ...
                                    @foreach($optionPaymentMethod as $v1)
                                        <option value="{{$v1['payment_method_id']}}" {{$infoPayment[$v['key']] == $v1['payment_method_id'] ? 'selected': ''}}>{{$v1['payment_method_name']}}</option>
                                    @endforeach
                                </select>
                            @elseif($v['key'] == 'payment_unit_id')
                                <select class="form-control select" id="{{$v['key']}}"
                                    ...
                                    @foreach($optionPaymentUnit as $v1)
                                        <option value="{{$v1['payment_unit_id']}}" {{$infoPayment[$v['key']] == $v1['payment_unit_id'] ? 'selected': ''}}>{{$v1['name']}}</option>
                                    @endforeach
                                </select>
                            @endif
                            @break
...
<h4>@lang('Tình trạng hợp đồng')</h4>
                ...
                <span class="m-switch m-switch--icon m-switch--success m-switch--sm">
                    <label>
                        <input id="is_created_ticket" name="is_created_ticket" type="checkbox"
                                {{$infoGeneral['is_created_ticket'] == 1 ? 'checked': ''}}>
                        @lang('Tạo ticket khi hợp đồng ở trạng thái')
                        <span></span>
                    </label>
                </span>
