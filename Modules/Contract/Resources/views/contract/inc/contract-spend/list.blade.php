<div class="table-responsive">
    <table class="table table-striped m-table m-table--head-bg-default">
        ...
        <tr>
            ...
            <th class="tr_thead_list">{{__('NGÀY CHI')}}</th>
            <th class="tr_thead_list">{{__('NGƯỜI CHI')}}</th>
            <th class="tr_thead_list">{{__('GIÁ TRỊ THANH TOÁN')}}</th>
            <th class="tr_thead_list">{{__('PHƯƠNG THỨC THANH TOÁN')}}</th>
        ...
        @if(isset($LIST))
            @foreach ($LIST as $key => $item)
                <tr>
                    <td>
                        @if(session()->get('is_detail') == 0)
                            <a href="javascript:void(0)"
                               onclick="contractSpend.showModalEdit('{{$item['contract_spend_id']}}')"
                               class="test m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill"
                               title="{{__('Sửa')}}">
                                <i class="la la-edit"></i>
                            </a>
                            <button onclick="contractSpend.showModalRemove('{{$item['contract_spend_id']}}')"
                    ...
                    <td>{{$item['content']}}</td>
                    <td>{{\Carbon\Carbon::parse($item['spend_date'])->format('d/m/Y')}}</td>
                    <td>{{$item['spend_by_name']}}</td>
                    <td>
                        {{number_format($item['amount_spend'], isset(config()->get('config.decimal_number')->value) ? config()->get('config.decimal_number')->value : 0)}}
                    </td>
