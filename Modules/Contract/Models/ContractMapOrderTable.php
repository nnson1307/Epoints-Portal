...
class ContractMapOrderTable extends Model
{
    ...
    /**
     * Lấy thông tin tất cả đơn hàng map với hợp đồng
     *
     * @param $contractCode
     * @return mixed
     */
    public function getAllContractMapOrder($contractCode)
    {
        return $this
            ->select(
                "{$this->table}.contract_map_order_id",
    ...
    /**
     * Huỷ liên kết đơn hàng với hợp đồng
     *
     * @param $mapOrderId
     * @return mixed
     */
    public function removeContractMapOrder($mapOrderId)
    {
        return $this->where("contract_map_order_id", $mapOrderId)->delete();
    }
}