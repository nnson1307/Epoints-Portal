<div class="m-datatable m-datatable--default">
    <div class="m-datatable__pager m-datatable--paging-loaded clearfix">
        ...
        <ul class="m-datatable__pager-nav" style="float: right">
            ...
            @else
                <li><a href="{{ \Request::url().'?year='.$year }}" title="First" class="m-datatable__pager-link m-datatable__pager-link--first" data-page="1"><i class="la la-angle-double-left"></i></a></li>
                <li><a href="{{ $paginator->previousPageUrl().'&year='.$year }}" title="Previous" class="m-datatable__pager-link m-datatable__pager-link--prev" data-page="{{ $paginator->currentPage() - 1 }}"><i class="la la-angle-left"></i></a></li>
            @endif
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                        	<li><a href="{{ $url.'&year='.$year.'&people_object_group_id='.$people_object_group_id}}" class="m-datatable__pager-link" data-page="{{ $page }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @else
                <li><a href="{{ $paginator->nextPageUrl().'&year='.$year }}" title="Next" class="m-datatable__pager-link m-datatable__pager-link--next" data-page="{{ $paginator->currentPage() + 1 }}"><i class="la la-angle-right"></i></a></li>
            	<li><a href="{{ \Request::url().'?page='.$paginator->lastPage().'&year='.$year }}" title="Last" class="m-datatable__pager-link m-datatable__pager-link--last" data-page="{{ $paginator->lastPage() }}"><i class="la la-angle-double-right"></i></a></li>
            @endif
        </ul>
        @endif
