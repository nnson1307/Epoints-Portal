        ...
        @if ($paginator->hasPages())
            <ul class="m-datatable__pager-nav" style="float: right">
                ...
                @else
                    ...
                    <li><a title="Previous" class="m-datatable__pager-link m-datatable__pager-link--prev" data-page="{{ $paginator->currentPage() - 1 }}"><i class="la la-angle-left"></i></a></li>
                @endif
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <li><a class="m-datatable__pager-link" data-page="{{ $page }}">{{ $page }}</a></li>
                            @endif
                        @endforeach
                    @endif
                @endforeach
                ...
                @else
                    <li><a title="Next" class="m-datatable__pager-link m-datatable__pager-link--next" data-page="{{ $paginator->currentPage() + 1 }}"><i class="la la-angle-right"></i></a></li>
                    <li><a title="Last" class="m-datatable__pager-link m-datatable__pager-link--last" data-page="{{ $paginator->lastPage() }}"><i class="la la-angle-double-right"></i></a></li>
                @endif
            </ul>
        @endif
        <div class="m-datatable__pager-info" style="float: left" >
            @php
                $display = [ 1=>1, 2=>2 , 10 => 10, 25 => 25, 50 => 50, 100 => 100];
                //$display = [1 => 1, 2 => 2];
            @endphp
            ...
            <select class="m-datatable__pager-info selectpicker d-block width70" name="cars" id="cars">
